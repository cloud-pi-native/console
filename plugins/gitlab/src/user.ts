...
export async function getUser(user: {
  ...
  username: string;
  id: string;
}): Promise<SimpleUserSchema | undefined> {
  const api = getApi();
...
export async function upsertUser(
  ...
  if (existingUser) {
    const incorrectProps = Object.entries(userDefinitionBase).reduce(
      (acc, [key, value]) => {
    ...
    if (incorrectProps.length) {
      logger.debug(
        { action: 'upsertUser', changes: incorrectProps },
        'User properties differ from expected',
      );
      try {
    ...
    try {
      ...
      logger.debug(
        { action: 'upsertUser', userId: existingUser.id, err },
        'Failed to upsert user custom attribute',
      );
    }
    return existingUser;
  }
  ...
  try {
    ...
    logger.debug(
      { action: 'upsertUser', userId: created.id, err },
      'Failed to upsert user custom attribute',
    );
  }
  return created;
