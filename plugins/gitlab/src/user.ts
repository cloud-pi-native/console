...
import { getApi, getAll, iter } from './utils.js'
...
export async function getUser(user: { email: string, username: string, id: string }): Promise<SimpleUserSchema | undefined> {
  ...
  const usersByEmail = await getAll(iter(opts => api.Users.all(opts)))
  ...
  const usersByUsername = await getAll(iter(opts => api.Users.all(opts)))
