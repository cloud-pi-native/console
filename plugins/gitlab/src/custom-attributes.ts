...
export async function upsertCustomAttribute(
  resource: 'groups' | 'projects' | 'users',
  id: number,
  key: string,
  value: string,
): Promise<void> {
  ...
  logger.debug(
    { action: 'upsertCustomAttribute', resource, id, key },
    'Upsert custom attribute details',
  );
  const api = getApi();
  try {
    if (resource === 'groups') {
    ...
    logger.warn(
      { action: 'upsertCustomAttribute', resource, id, key, err },
      'Failed to upsert custom attribute',
    );
    throw err;
  }
