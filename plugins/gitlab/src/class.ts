...
type AccessLevelAllowed =
  | AccessLevel.NO_ACCESS
  | AccessLevel.MINIMAL_ACCESS
  | AccessLevel.GUEST
...
export class GitlabApi extends PluginApi {
  ...
  public async createEmptyRepository({
    createFirstCommit,
    groupId,
    repoName,
    description,
    ciConfigPath,
  }: CreateEmptyRepositoryArgs & {
    createFirstCommit: boolean;
    groupId: number;
    ciConfigPath?: string;
  }) {
    logger.debug(
      { action: 'createEmptyRepository', repoName, groupId, createFirstCommit, ciConfigPath },
    ...
    try {
      ...
      logger.debug(
        { action: 'createEmptyRepository', projectId: project.id, err },
        'Failed to upsert project custom attribute',
      );
    }
  ...
  public async commitCreateOrUpdate(
    ...
    logger.debug(
      { action: 'commitCreateOrUpdate', repoId, filePath, branch },
      'Schedule commit create/update',
    );
    let action: CommitAction['action'] = 'create';

    const existingBranch = await find(
      offsetPaginate((opts) => this.api.Branches.all(repoId, opts), {
  ...
  public async commitDelete(
    ...
    logger.debug(
      { action: 'commitDelete', repoId, branch, filesCount: files.length },
      'Schedule commit delete',
    );
    if (files.length) {
    ...
    for (const [id, repo] of objectEntries(this.pendingCommits)) {
      for (const [branch, details] of objectEntries(repo.branches)) {
        ...
        if (filesNumber) {
          ...
          const message = [
            `ci: :robot_face: Update ${filesNumber} file${filesNumber > 1 ? 's' : ''}`,
            ...details.messages.filter((m) => m),
          ].join('\n');
          logger.debug(
            { action: 'commitFiles', repoId: id, branch, filesNumber },
            'Commit pending file changes',
          );
  ...
  public async deleteRepository(repoId: number, fullPath: string) {
    ...
    return this.api.Projects.remove(repoId, {
      ...
      fullPath: `${fullPath}-deletion_scheduled-${repoId}`,
    }); // Effective deletion
  }
}
...
export class GitlabZoneApi extends GitlabApi {
  ...
  public async getOrCreateInfraGroup(): Promise<GroupSchema> {
    ...
    const group =
      existingParentGroup ||
      (await this.api.Groups.create(infraGroupName, infraGroupPath, {
        parentId: rootId,
        projectCreationLevel: 'maintainer',
        subgroupCreationLevel: 'owner',
        defaultBranchProtection: 0,
        description:
          'Group that hosts infrastructure-as-code repositories for all zones (ArgoCD pull targets).',
      }));
    try {
      ...
      logger.debug(
        { action: 'getOrCreateInfraGroup', groupId: group.id, err },
        'Failed to upsert infra group custom attribute',
      );
    }
    return group;
  }
  ...
  public async getOrCreateInfraProject(zone: string): Promise<ProjectSchema> {
    ...
    const fast = await find(
      offsetPaginate((opts) =>
        this.api.Groups.allProjects(infraGroup.id, {
...
export class GitlabProjectApi extends GitlabApi {
  ...
  private async createProjectGroup(): Promise<GroupSchema> {
    logger.info(
      { action: 'createProjectGroup', projectSlug: this.project.slug },
    ...
    try {
      ...
      logger.debug(
        { action: 'createProjectGroup', groupId: group.id, err },
        'Failed to upsert project group custom attribute',
      );
    }
    return group;
  }
  ...
  public async getProjectGroup(): Promise<GroupSchema | undefined> {
    logger.debug(
      { action: 'getProjectGroup', projectSlug: this.project.slug },
      'Get project group',
    );
    if (!this.gitlabGroup) {
      logger.debug(
      ...
      const fast = await find(
        offsetPaginate((opts) =>
          this.api.Groups.allSubgroups(parentId, {
      ...
      if (this.gitlabGroup) {
        try {
          ...
          logger.debug(
            { action: 'getProjectGroup', groupId: this.gitlabGroup.id, err },
            'Failed to upsert project group custom attribute',
          );
        }
      }
  ...
  public async getOrCreateProjectGroup(): Promise<GroupSchema> {
    logger.debug(
      { action: 'getOrCreateProjectGroup', projectSlug: this.project.slug },
      'Get/create project group',
    );
  ...
  public async getProjectMirrorCreds(vaultApi: VaultProjectApi): Promise<GitlabMirrorSecret> {
    logger.debug(
      { action: 'getProjectMirrorCreds', projectSlug: this.project.slug },
      'Get/create project mirror credentials',
    );
    ...
    if (currentToken) {
      const vaultSecret = (await vaultApi.read('tech/GITLAB_MIRROR', {
        throwIfNoEntry: false,
      })) as { data: GitlabMirrorSecret };
      if (vaultSecret) {
        try {
          const group = await this.getProjectGroup();
          ...
          logger.warn(
            { action: 'getProjectMirrorCreds', err: error },
            'Mirror token invalid, revoking project token',
          );
          await this.revokeProjectToken(currentToken.id);
        }
      }
    }
    const newToken = await this.createProjectToken(tokenName, [
      'write_repository',
      'read_repository',
      'read_api',
    ]);
  ...
  public async getProjectId(projectName: string) {
    logger.debug(
      { action: 'getProjectId', projectName, projectSlug: this.project.slug },
      'Look up project id',
    );
    const projectGroup = await this.getProjectGroup();
    if (!projectGroup)
      throw new Error(`Gitlab inaccessible, impossible de trouver le groupe ${this.project.slug}`);

    const fast = await find(
      offsetPaginate((opts) =>
        this.api.Groups.allProjects(projectGroup.id, {
          ...customAttributesFilter(managedByConsoleCustomAttributeKey, 'true'),
          search: projectName,
          simple: true,
          ...opts,
  ...
  public async getProjectToken(tokenName: string) {
    ...
    return find(
      offsetPaginate((opts) => this.api.GroupAccessTokens.all(group.id, opts), {
        perPage: MAX_PAGINATION_PER_PAGE,
      }),
  ...
  public async createProjectToken(tokenName: string, scopes: AccessTokenScopes[]) {
    logger.info(
      { action: 'createProjectToken', tokenName, projectSlug: this.project.slug },
    ...
    return this.api.GroupAccessTokens.create(
      group.id,
      tokenName,
      scopes,
  ...
  public async revokeProjectToken(tokenId: number) {
    logger.info(
      { action: 'revokeProjectToken', tokenId, projectSlug: this.project.slug },
  ...
  public async getMirrorProjectTriggerToken(vaultApi: VaultProjectApi) {
    logger.debug(
      { action: 'getMirrorProjectTriggerToken', projectSlug: this.project.slug },
      'Get mirror project trigger token',
    );
    ...
    const currentTriggerToken = await find(
      offsetPaginate((opts) => this.api.PipelineTriggerTokens.all(mirrorRepo.id, opts), {
        perPage: MAX_PAGINATION_PER_PAGE,
      }),
    ...
    const triggerToken = await this.api.PipelineTriggerTokens.create(
      mirrorRepo.id,
      tokenDescription,
    );
  ...
  public async listRepositories() {
    ...
    const projects = await getAll(
      offsetPaginate((opts) => this.api.Groups.allProjects(group.id, { simple: false, ...opts }), {
        perPage: MAX_PAGINATION_PER_PAGE,
      }),
    ); // to refactor with https://github.com/jdalrymple/gitbeaker/pull/3624
    return Promise.all(
      projects.map(async (project) => {
        if (
          this.specialRepositories.includes(project.name) &&
          (!project.topics || !project.topics.includes(pluginManagedTopic))
        ) {
          return this.api.Projects.edit(project.id, {
            topics: project.topics ? [...project.topics, pluginManagedTopic] : [pluginManagedTopic],
          });
        }
        return project;
      }),
  ...
  public async createEmptyProjectRepository({
    repoName,
    description,
    clone,
  }: CreateEmptyRepositoryArgs & { clone?: boolean }) {
    logger.info(
      { action: 'createEmptyProjectRepository', repoName, projectSlug: this.project.slug, clone },
      'Create empty project repository',
    );
  ...
  public async addSpecialRepositories(name: string) {
    logger.debug(
      { action: 'addSpecialRepositories', name, projectSlug: this.project.slug },
      'Register special repository',
    );
  ...
  public async getGroupMembers() {
    const group = await this.getOrCreateProjectGroup();
    return getAll(
      offsetPaginate((opts) => this.api.GroupMembers.all(group.id, opts), {
        perPage: MAX_PAGINATION_PER_PAGE,
      }),
    );
  }
  ...
  public async addGroupMember(
    userId: number,
    accessLevel: AccessLevelAllowed = AccessLevel.DEVELOPER,
  ): Promise<MemberSchema> {
    logger.info(
      { action: 'addGroupMember', accessLevel, projectSlug: this.project.slug },
      'Add group member',
    );
    const group = await this.getOrCreateProjectGroup();
  ...
  public async editGroupMember(
    userId: number,
    accessLevel: AccessLevelAllowed = AccessLevel.DEVELOPER,
  ): Promise<MemberSchema> {
    logger.info(
      { action: 'editGroupMember', accessLevel, projectSlug: this.project.slug },
      'Edit group member',
    );
    const group = await this.getOrCreateProjectGroup();
  ...
  public async removeGroupMember(userId: number) {
    logger.info(
      { action: 'removeGroupMember', projectSlug: this.project.slug },
      'Remove group member',
    );
    const group = await this.getOrCreateProjectGroup();
  ...
  public async getGitlabGroupVariables(): Promise<VariableSchema[]> {
    const group = await this.getOrCreateProjectGroup();
    return await getAll(
      offsetPaginate((opts) => this.api.GroupVariables.all(group.id, opts), {
        perPage: MAX_PAGINATION_PER_PAGE,
      }),
    );
  }
  ...
  public async setGitlabGroupVariable(
    listVars: VariableSchema[],
    toSetVariable: VariableSchema,
    ...
    if (currentVariable) {
      if (
        ...
        await this.api.GroupVariables.edit(group.id, toSetVariable.key, toSetVariable.value, {
    ...
    await this.api.GroupVariables.create(group.id, toSetVariable.key, toSetVariable.value, {
      variableType: toSetVariable.variable_type,
      masked: toSetVariable.masked,
      protected: toSetVariable.protected,
    });
  ...
  public async getGitlabRepoVariables(repoId: number): Promise<VariableSchema[]> {
    return await getAll(
      offsetPaginate((opts) => this.api.ProjectVariables.all(repoId, opts), {
        perPage: MAX_PAGINATION_PER_PAGE,
      }),
    );
  }
  ...
  public async setGitlabRepoVariable(
    repoId: number,
    listVars: VariableSchema[],
    toSetVariable: ProjectVariableSchema,
  ): Promise<setVariableResult | 'repository not found'> {
    const currentVariable = listVars.find((v) => v.key === toSetVariable.key);
    if (currentVariable) {
      if (
        currentVariable.masked !== toSetVariable.masked ||
        ...
        await this.api.ProjectVariables.edit(repoId, toSetVariable.key, toSetVariable.value, {
          variableType: toSetVariable.variable_type,
          masked: toSetVariable.masked,
          protected: toSetVariable.protected,
          filter: {
    ...
    await this.api.ProjectVariables.create(repoId, toSetVariable.key, toSetVariable.value, {
      variableType: toSetVariable.variable_type,
      masked: toSetVariable.masked,
      protected: toSetVariable.protected,
    });
