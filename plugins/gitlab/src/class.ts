...
export class GitlabZoneApi extends GitlabApi {
  ...
  public async getOrCreateInfraGroup(): Promise<GroupSchema> {
    ...
    const existingParentGroup = await find(offsetPaginate(opts => this.api.Groups.all({ ...opts, search: infraGroupName })), group => group.parent_id === rootId && group.name === infraGroupName)
...
export class GitlabProjectApi extends GitlabApi {
  ...
  private async createProjectGroup(): Promise<GroupSchema> {
    ...
    const existingGroup = await find(offsetPaginate(opts => this.api.Groups.all({ ...opts, search: this.project.slug })), group => group.parent_id === parentId && group.name === this.project.slug)
