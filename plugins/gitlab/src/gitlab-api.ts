...
export abstract class GitlabApi extends PluginApi {
  ...
  public async commitCreateOrUpdate(
    ...
    if (branches.some(b => b.name === branch)) {
      ...
      try {
        actualFile = await this.api.RepositoryFiles.show(
          repoId,
          filePath,
          branch,
        )
      ...
      if (actualFile) {
        const newContentDigest = createHash('sha256')
          .update(fileContent)
          .digest('hex')
  ...
  private addActions(
    ...
    if (this.pendingCommits[repoId].branches[branch]) {
      this.pendingCommits[repoId].branches[branch].actions.push(
        ...commitActions,
      )
  ...
  public async listFiles(
    repoId: number,
    options: AllRepositoryTreesOptions & PaginationRequestOptions<'keyset'> = {},
  ) {
    options.path = options?.path ?? '/'
    options.ref = options?.ref ?? 'main'
    options.recursive = options?.recursive ?? false
    try {
      const files: RepositoryTreeSchema[]
        = await this.api.Repositories.allRepositoryTrees(repoId, options)
      return files
    } catch (error) {
