...
export const upsertProjectMember: StepCall<ProjectMember> = async (payload) => {
  ...
  const projectReporterGroupPath = config?.projectReporterGroupPath
  if (!projectReporterGroupPath) {
    throw new Error('projectReporterGroupPath is required')
  }
  const projectDeveloperGroupPath = config?.projectDeveloperGroupPath
  if (!projectDeveloperGroupPath) {
    throw new Error('projectDeveloperGroupPath is required')
  }
  const projectMaintainerGroupPath = config?.projectMaintainerGroupPath
  if (!projectMaintainerGroupPath) {
    throw new Error('projectMaintainerGroupPath is required')
  }
  ...
  try {
    ...
    if (member.roles.find(role => role.oidcGroup && matchRole(member.project.slug, role.oidcGroup, projectReporterGroupPath))) {
      maxAccessLevel = AccessLevel.GUEST
    } else if (member.roles.find(role => role.oidcGroup && matchRole(member.project.slug, role.oidcGroup, projectDeveloperGroupPath))) {
      maxAccessLevel = AccessLevel.DEVELOPER
    } else if (member.roles.find(role => role.oidcGroup && matchRole(member.project.slug, role.oidcGroup, projectMaintainerGroupPath))) {
