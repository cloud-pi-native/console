...
import { DEFAULT_ADMIN_GROUP_PATH, DEFAULT_AUDITOR_GROUP_PATH } from './infos.js';
...
export const upsertDsoProject: StepCall<Project> = async (payload) => {
  ...
  try {
    ...
    await Promise.all(
      project.users.map((user) => ensureGroup(gitlabApi, project, user, payload.config)),
    );
    ...
    const destroySecrets = (await vaultApi.list())
      ...
      .filter(
        (repoName) =>
          !project.repositories.some((projectRepo) => projectRepo.internalRepoName === repoName),
      );

    await Promise.all(destroySecrets.map((repoName) => vaultApi.destroy(`${repoName}-mirror`)));
...
export const commitFiles: StepCall<UniqueRepo | Project | ClusterObject | ZoneObject> = async (
  payload,
) => {
