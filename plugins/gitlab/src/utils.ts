...
import type { Gitlab as IGitlab, BaseRequestOptions, GroupSchema, PaginationRequestOptions, OffsetPagination } from '@gitbeaker/core'
...
export async function getGroupRootId(throwIfNotFound?: boolean): Promise<number | undefined> {
  ...
  const groupRoot = await find(offsetPaginate(opts => gitlabApi.Groups.all({
    ...
    orderBy: 'id',
    ...opts,
  })), (grp: GroupSchema) => grp.full_path === projectRootDir)
