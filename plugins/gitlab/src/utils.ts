...
export async function getGroupRootId(throwIfNotFound?: boolean): Promise<number | undefined> {
  ...
  logger.debug(
    { action: 'getGroupRootId', groupRootId: groupRoot?.id, groupRootPath: groupRoot?.full_path },
    'Resolved group root',
  );
  ...
  try {
    ...
    logger.debug(
      { action: 'getGroupRootId', groupRootId, err },
      'Failed to upsert group root custom attribute',
    );
  }
  return groupRootId;
}
...
async function createGroupRoot(): Promise<number> {
  ...
  if (parentGroup.full_path === projectRootDir) {
    try {
      ...
      logger.debug(
        { action: 'createGroupRoot', groupRootId: parentGroup.id, err },
        'Failed to upsert group root custom attribute',
      );
    }
