...
export async function ensureRepositories(
  ...
  const promises: Promise<any>[] = [
    ...
    ...gitlabRepositories
      ...
      .map((gitlabRepository) =>
        gitlabApi.deleteRepository(gitlabRepository.id, gitlabRepository.path_with_namespace),
      ),
    // create missing repositories
    ...project.repositories.map((repo) =>
      ensureRepositoryExists(gitlabRepositories, repo, gitlabApi, projectMirrorCreds, vaultApi),
    ),
  ];
  ...
  if (!gitlabRepositories.some((repo) => repo.name === internalMirrorRepoName)) {
    promises.push(
      gitlabApi
        .createEmptyProjectRepository({ repoName: internalMirrorRepoName, clone: false })
...
async function ensureRepositoryExists(
  ...
  const gitlabRepository: CondensedProjectSchema | ProjectSchema | void = gitlabRepositories.find(
    (gitlabRepository) => gitlabRepository.name === repository.internalRepoName,
  );
  ...
  if (!currentVaultSecret?.data || !shallowEqual(mirrorSecretData, currentVaultSecret.data)) {
