...
export function getTechUsed(payload: HookPayload<Project | ProjectLite>) {
  ...
  if (specificallyEnabled(projectStore?.activateMavenRepo)) {
    ...
    defaultOrNullish(projectStore?.activateMavenRepo) &&
    specificallyEnabled(globalConfig?.activateMavenRepoDefaultValue)
  ) {
    techUsed.maven = true;
  }
...
export function parseProjectOptions(options: ProjectLite['store']['nexus']) {
  // @ts-expect-error
  const mavenReleaseWritePolicy = parseWritePolicy(
    options?.mavenReleaseWritePolicy ?? options?.mavenWritePolicy,
  );
  // @ts-expect-error
  const mavenSnapshotWritePolicy = parseWritePolicy(
    options?.mavenSnapshotWritePolicy ?? options?.mavenWritePolicy,
  );
