...
const ADMIN_TEMPLATE_NAME = 'Default admin template'
const READONLY_TEMPLATE_NAME = 'Default readonly template'
...
export const upsertAdminRole: StepCall<AdminRole> = async (payload) => {
  try {
    ...
    if (role.oidcGroup === adminGroupPath) {
      ...
      await setTemplateGroupPermissions(adminGroupPath, adminProjectPermissions, ADMIN_TEMPLATE_NAME)
      ...
      await setTemplateGroupPermissions(readonlyGroupPath, readonlyProjectPermissions, READONLY_TEMPLATE_NAME)
    }
...
async function ensureAdminTemplateExists() {
  ...
  await setTemplateGroupPermissions('sonar-administrators', adminProjectPermissions, ADMIN_TEMPLATE_NAME)
}
...
async function ensureReadonlyTemplateExists() {
  ...
  await axiosInstance({
    method: 'post',
    params: { name: READONLY_TEMPLATE_NAME },
    url: 'permissions/create_template',
    validateStatus: code => [200, 400].includes(code),
  })

  // Add Project Creator and sonar-administrators to Readonly Template
  await Promise.all(readonlyProjectPermissions.map(permission =>
    axiosInstance({
      method: 'post',
      params: {
        templateName: READONLY_TEMPLATE_NAME,
        permission,
      },
      url: 'permissions/add_project_creator_to_template',
    }),
  ))
  await setTemplateGroupPermissions('sonar-administrators', readonlyProjectPermissions, READONLY_TEMPLATE_NAME)
