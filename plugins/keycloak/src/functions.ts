  ...
  try {
    ...
    for (const environment of project.environments) {
      const envGroup: Required<CustomGroup> =
        (envGroups.find((group) => group.name === environment.name) as Required<CustomGroup>) ??
      ...
      for (const permission of environment.permissions) {
        if (permission.permissions.ro) {
          ...
          promises.push(
            kcClient.users.delFromGroup({ id: permission.userId, groupId: roGroup.id }),
          );
        }
        if (permission.permissions.rw) {
          ...
          promises.push(
            kcClient.users.delFromGroup({ id: permission.userId, groupId: rwGroup.id }),
          );
        }
      }
    }
...
export const deleteZone: StepCall<ZoneObject> = async ({ args: zone }) => {
  ...
  try {
    ...
    logger.info(
      { action: 'deleteZone', zoneSlug, clientId, outcome: 'already-missing' },
      'Hook done',
    );
    return {
      status: {
    ...
    logger.error(
      { action: 'deleteZone', zoneSlug, clientId: getClientZoneId(zone), err: error },
      'Hook failed',
    );
    return {
      error,
...
export const upsertAdminRole: StepCall<AdminRole> = async ({ args: role }) => {
  ...
  try {
    ...
    logger.info(
      { action: 'upsertAdminRole', roleId: role.id, oidcGroup: role.oidcGroup },
      'Hook done',
    );
    return pluginResult.getResultObject();
  } catch (error) {
    logger.error(
      { action: 'upsertAdminRole', roleId: role.id, oidcGroup: role.oidcGroup, err: error },
      'Hook failed',
    );
    return pluginResult.returnUnexpectedError(error);
  }
...
export const deleteAdminRole: StepCall<AdminRole> = async ({ args: role }) => {
  ...
  try {
    ...
    logger.info(
      { action: 'deleteAdminRole', roleId: role.id, oidcGroup: role.oidcGroup },
      'Hook done',
    );
    return pluginResult.getResultObject();
  } catch (error) {
    logger.error(
      { action: 'deleteAdminRole', roleId: role.id, oidcGroup: role.oidcGroup, err: error },
      'Hook failed',
    );
    return pluginResult.returnUnexpectedError(error);
  }
...
export const upsertProjectRole: StepCall<ProjectRole> = async ({ args: role }) => {
  ...
  try {
    ...
    logger.info(
      { action: 'upsertProjectRole', roleId: role.id, oidcGroup: role.oidcGroup },
      'Hook done',
    );
    return {
      status: {
    ...
    logger.error(
      { action: 'upsertProjectRole', roleId: role.id, oidcGroup: role.oidcGroup, err: error },
      'Hook failed',
    );
    return {
      error,
...
export const deleteProjectRole: StepCall<ProjectRole> = async ({ args: role }) => {
  ...
  try {
    ...
    if (projectGroup?.id) {
      ...
      const pluginGroup = pluginGroups.find(({ name }) => name === pluginName) as
        | Required<GroupRepresentation>
        | undefined;
      if (pluginGroup?.id) {
        const roleGroups = await getAllSubgroups(kcClient, pluginGroup.id, 0);
        const roleGroup = roleGroups.find(({ name }) => name === roleName) as
          | Required<GroupRepresentation>
          | undefined;
        if (roleGroup?.id) {
    ...
    logger.error(
      { action: 'deleteProjectRole', roleId: role.id, oidcGroup: role.oidcGroup, err: error },
      'Hook failed',
    );
    return {
      error,
...
export const upsertProjectMember: StepCall<ProjectMember> = async ({ args: member }) => {
  ...
  try {
    ...
    logger.info(
      { action: 'upsertProjectMember', projectSlug: member.project.slug, groupMembershipChanges },
      'Hook done',
    );
    return pluginResult.getResultObject();
  } catch (error) {
    logger.error(
      { action: 'upsertProjectMember', projectSlug: member.project.slug, err: error },
      'Hook failed',
    );
    return pluginResult.returnUnexpectedError(error);
  }
...
export const deleteProjectMember: StepCall<ProjectMember> = async ({ args: member }) => {
  ...
  try {
    ...
    logger.info(
      { action: 'deleteProjectMember', projectSlug: member.project.slug, removedCount },
      'Hook done',
    );
    return pluginResult.getResultObject();
  } catch (error) {
    logger.error(
      { action: 'deleteProjectMember', projectSlug: member.project.slug, err: error },
      'Hook failed',
    );
    return pluginResult.returnUnexpectedError(error);
  }
