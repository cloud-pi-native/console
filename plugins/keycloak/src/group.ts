...
export type CustomGroup = Required<
  Pick<GroupRepresentation, 'id' | 'name' | 'subGroups' | 'subGroupCount'>
>;
export async function getGroupByName(
  kcClient: KeycloakAdminClient,
  name: string,
): Promise<GroupRepresentation | void> {
...
export async function getAllSubgroups(
  ...
  const newSubgroups = [
    ...subgroups,
    ...(await kcClient.groups.listSubGroups({
      parentId,
...
export async function getOrCreateChildGroup(
  kcClient: KeycloakAdminClient,
  parentId: string,
  name: string,
  ...
  if (Array.isArray(subGroups) && subGroups.length > 0) {
    const matchingGroup = subGroups.find(({ name: groupName }) => groupName === name) as
...
export async function getOrCreateProjectGroup(
  kcClient: KeycloakAdminClient,
  name: string,
): Promise<
  Required<Pick<GroupRepresentation, 'id' | 'name' | 'subGroups' | 'subGroupCount'>> & {
    subGroups: Required<GroupRepresentation>[];
  }
> {
...
export async function getOrCreateGroupByName(
  kcClient: KeycloakAdminClient,
  name: string,
): Promise<Required<GroupRepresentation>> {
...
export async function getOrCreateGroupByPath(
  kcClient: KeycloakAdminClient,
  path: string,
): Promise<Required<GroupRepresentation>> {
  if (!path.startsWith('/')) {
