...
export const createDsoProject: StepCall<Project> = async (payload) => {
  ...
  try {
    ...
    const createProjectRobot =
      ...
      (specificallyEnabled(publishRoRobotConfig) && !specificallyDisabled(publishRoRobotProject));

    const quotaHardLimit =
      project.store.registry?.quotaHardLimit || payload.config.registry?.quotaHardLimit;
    const quotaHardLimitBytes = quotaHardLimit ? bytes.parse(quotaHardLimit) : undefined;
    ...
    logger.info(
      {
        action: 'createDsoProject',
        projectSlug: projectName,
        result: returnResult.status.result,
        createProjectRobot,
      },
      'Hook done',
    );
    ...
    logger.error(
      { action: 'createDsoProject', projectSlug: payload.args.slug, err: error },
      'Hook failed',
    );
    return {
      error,
...
export const deleteDsoProject: StepCall<Project> = async (payload) => {
  try {
    ...
    logger.info(
      { action: 'deleteDsoProject', projectSlug: projectName, outcome: 'deleted' },
      'Hook done',
    );
    return {
      status: {
    ...
    logger.error(
      { action: 'deleteDsoProject', projectSlug: payload.args.slug, err: error },
      'Hook failed',
    );
    return {
      error,
