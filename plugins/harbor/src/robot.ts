...
export const getRobot = async (projectName: string, robotName: string, api: HarborApi) =>
  getRobotByName(projectName, `robot$${projectName}+${robotName}`, api);
...
export async function ensureRobot(
  ...
  access: Access[],
  api: HarborApi,
): Promise<VaultRobotSecret> {
  logger.debug(
    { action: 'ensureRobot', projectSlug: projectName, robotName },
    'Ensuring Harbor robot',
  );
  const vaultPath = `REGISTRY/${robotName}`;
  const robot = await getRobot(projectName, robotName, api);
  const vaultRobotSecret = (await vaultApi.read(vaultPath, { throwIfNoEntry: false })) as
    | { data: VaultRobotSecret }
    | undefined;
  ...
  if (vaultRobotSecret?.data && vaultRobotSecret.data.HOST === getConfig().host) {
    ...
    creds = toVaultSecret(
      (await regenerateRobot(projectName, robotName, access, api)) as Required<RobotCreated>,
    );
    await vaultApi.write(creds, vaultPath);
  } else {
    creds = toVaultSecret(
      (await createRobot(projectName, robotName, access, api)) as Required<RobotCreated>,
    );
    await vaultApi.write(creds, vaultPath);
  }
  return creds;
...
export async function deleteRobot(
  ...
  logger.info(
    { action: 'deleteRobot', projectSlug: projectName, robotName },
    'Deleting Harbor robot',
  );
  const vaultPath = `REGISTRY/${robotName}`;
  const robot = await getRobot(projectName, robotName, api);
  if (robot?.id) {
...
export async function createRobot(
  ...
  logger.info(
    { action: 'createRobot', projectSlug: projectName, robotName },
    'Creating Harbor robot',
  );
  return (await api.robots.createRobot(getRobotPermissions(projectName, robotName, access))).data;
}

export async function regenerateRobot(
  projectName: string,
  robotName: string,
  access: Access[],
  api: HarborApi,
  ...
  logger.info(
    { action: 'regenerateRobot', projectSlug: projectName, robotName },
    'Regenerating Harbor robot',
  );
  const robot = await getRobot(projectName, robotName, api);
...
export async function getRobotByName(
  project: string | number,
  robotName: string,
  api: HarborApi,
): Promise<Robot | undefined> {
  logger.debug(
    { action: 'getRobotByName', projectRef: project, robotName },
    'Listing Harbor robots to find matching name',
  );
