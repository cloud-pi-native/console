...
const quotaDescription =
  '-1 -> illimité, sinon 100MB / 1.2GB (unités : B, KB, MB, GB, TB)), max 1024TB';
const infos = {
  name: 'registry',
  to: ({ store }) =>
    store?.registry?.projectId
      ? `${getConfig().url}/harbor/projects/${store.registry.projectId}`
      : `${getConfig().url}/`,
  title: 'Harbor',
  ...
  config: {
    project: [
      ...
      },
      {
        ...
        description:
          'Activer le robot de projet (read-only) et afficher ses identifiants aux utilisateurs',
        permissions: {
