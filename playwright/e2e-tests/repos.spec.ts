...
import { clientURL, signInCloudPiNative, tcolinUser, testUser } from '../config/console';
...
test.describe('Repositories', () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await expect(page.locator('h2')).toContainText('Ajouter un dépôt au projet');
      ...
      await page.getByTestId('standaloneRepoSwitch').locator('input').uncheck({ force: true });
      await page.getByTestId('externalRepoUrlInput').fill(repo.externalRepoUrl);
      await expect(page.getByTestId('addRepoBtn')).toBeEnabled();
      await page.getByTestId('internalRepoNameInput').fill('$%_>');
      await expect(
        page.getByTestId('internalRepoNameInput').locator('//following-sibling::*[1]'),
      ...
      await expect(page.getByTestId('addRepoBtn')).toBeEnabled();
      await page.getByTestId('externalRepoUrlInput').fill(repo.externalRepoUrl.slice(0, -4));
      await expect(
        page.getByTestId('externalRepoUrlInput').locator('//following-sibling::*[1]'),
      ).toContainText(invalidGitUrlErrorMessage);
      await expect(page.getByTestId('addRepoBtn')).toBeDisabled();
      await page.getByTestId('externalRepoUrlInput').fill(repo.externalRepoUrl.slice(8));
      await expect(
        page.getByTestId('externalRepoUrlInput').locator('//following-sibling::*[1]'),
      ).toContainText(invalidGitUrlErrorMessage);
      await expect(page.getByTestId('addRepoBtn')).toBeDisabled();
      ...
      await expect(page.getByTestId('addRepoBtn')).toBeDisabled();
      await page.getByTestId('externalUserNameInput').fill(repo.externalUserName);
      await expect(page.getByTestId('addRepoBtn')).toBeEnabled();
  ...
  test('Should add a standalone public repo', { tag: '@e2e' }, async ({ page }) => {
    // Arrange
    await page.goto(clientURL);
    await signInCloudPiNative({ page, credentials: testUser });
    const { name: projectName, slug: projectSlug } = await createProject({
    ...
    await expect(page.locator('h2')).toContainText('Ajouter un dépôt au projet');
    await page.getByTestId('internalRepoNameInput').fill(repo.internalRepoName);
    await page.getByTestId('standaloneRepoSwitch').locator('input').check({ force: true });
    await page.getByTestId('addRepoBtn').click();
    await page.getByTestId(`repoTr-${repo.internalRepoName}`).click();
    await expect(page.locator('h2')).not.toContainText('Synchroniser le dépôt');
  });

  test('Should add an external public repo', { tag: '@e2e' }, async ({ page }) => {
    // Arrange
    await page.goto(clientURL);
    await signInCloudPiNative({ page, credentials: testUser });
    const { name: projectName, slug: projectSlug } = await createProject({
    ...
    await page.getByTestId('externalRepoUrlInput').fill(repo.externalRepoUrl);
    await page.getByTestId('addRepoBtn').click();
    await expect(page.getByTestId(`repoTr-${repo.internalRepoName}`)).toBeVisible();
  });

  test('Should add an external private repo', { tag: '@e2e' }, async ({ page }) => {
    // Arrange
    await page.goto(clientURL);
    await signInCloudPiNative({ page, credentials: testUser });
    const { name: projectName, slug: projectSlug } = await createProject({
    ...
    await page.getByTestId('internalRepoNameInput').fill(repo.internalRepoName);
    await page.getByTestId('externalRepoUrlInput').fill(repo.externalRepoUrl);
    await page.getByTestId('input-checkbox-privateRepoCbx').check({ force: true });
    await page.getByTestId('externalTokenInput').fill(repo.externalToken);
    await page.getByTestId('addRepoBtn').click();
    await expect(page.getByTestId(`repoTr-${repo.internalRepoName}`)).toBeVisible();
  });

  test('Should add an external public infra repo', { tag: '@e2e' }, async ({ page }) => {
    // Arrange
    await page.goto(clientURL);
    await signInCloudPiNative({ page, credentials: testUser });
    const { name: projectName, slug: projectSlug } = await createProject({
  ...
  test('Should add an external private infra repo', { tag: '@e2e' }, async ({ page }) => {
    // Arrange
    await page.goto(clientURL);
    await signInCloudPiNative({ page, credentials: testUser });
    const { name: projectName, slug: projectSlug } = await createProject({
  ...
  test('Should update a repo', { tag: '@e2e' }, async ({ page }) => {
    ...
    await page.getByTestId('internalRepoNameInput').fill(repo.internalRepoName);
    await page.getByTestId('externalRepoUrlInput').fill(repo.externalRepoUrl);
    await page.getByTestId('input-checkbox-infraRepoCbx').check({ force: true });
    await page.getByTestId('addRepoBtn').click();
    await expect(page.getByTestId(`repoTr-${repo.internalRepoName}`)).toBeVisible();
    ...
    await expect(page.getByTestId('input-checkbox-privateRepoCbx')).toBeChecked();
    await expect(page.getByTestId('externalUserNameInput')).toHaveValue('newUser');
  ...
  test('Should synchronise a repo', { tag: '@e2e' }, async ({ page }) => {
    ...
    await page.getByTestId('toggleSyncAllBranches').locator('input').check({ force: true });
    await expect(page.getByTestId('branchNameInput')).not.toBeVisible();
    await expect(page.getByTestId('syncRepoBtn')).toBeEnabled();
    await page.getByTestId('toggleSyncAllBranches').locator('input').uncheck({ force: true });
    await expect(page.getByTestId('branchNameInput')).toHaveValue('main');
    await expect(page.getByTestId('branchNameInput')).toBeEnabled();
    await expect(page.getByTestId('syncRepoBtn')).toBeEnabled();
  ...
  test('Should delete a repo', { tag: '@e2e' }, async ({ page }) => {
    ...
    await page.reload();
    await expect(page.getByTestId(`repoTr-${repo.internalRepoName}`)).not.toBeVisible();
  });
