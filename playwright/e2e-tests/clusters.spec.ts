...
test.describe('Clusters page', () => {
  test('should create a public cluster', { tag: '@e2e' }, async ({ page }) => {
    ...
    // Validate
    await expect(page.getByTestId(`clusterZone-${clusterName}`)).toContainText('publique');
    await expect(page.getByTestId(`clusterPrivacy-${clusterName}`)).toContainText('Public');
  });
  ...
  test('should create a dedicated cluster', { tag: '@e2e' }, async ({ page }) => {
    ...
    // Validate
    await expect(page.getByTestId(`clusterPrivacy-${clusterName}`)).toContainText('Dédié');
  });
