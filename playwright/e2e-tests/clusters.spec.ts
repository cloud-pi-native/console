...
import { addEnvToProject, addProject, createCluster, deleteCluster, waitForCluster, waitForClusterSync } from './utils'
...
test.describe('Clusters page', () => {
  test('should create a public cluster', { tag: '@e2e' }, async ({ page }) => {
    ...
    await waitForCluster({ page, name: clusterName, timeoutMs: 60000 })
  ...
  test('should update a public cluster', { tag: '@e2e' }, async ({ page }) => {
    ...
    await waitForCluster({ page, name: clusterName, timeoutMs: 60000 })
    await Promise.all([
      page.waitForURL(adminClusterDetailsUrlRegex),
      page.getByTestId(`clusterLink-${clusterName}`).click(),
    ])
    ...
    await Promise.all([
      waitForClusterSync({ page }),
      page.getByTestId('updateClusterBtn').click(),
    ])
