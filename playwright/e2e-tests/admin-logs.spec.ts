...
test.describe('Admin - Logs', () => {
  test('should display logs and paginate', { tag: '@e2e' }, async ({ page }) => {
    ...
    await expect(page.getByTestId('positionInfo')).toBeVisible()

    await expect(page.getByTestId('seePreviousPageBtn')).toBeDisabled()
    await expect(page.getByTestId('seeFirstPageBtn')).toBeDisabled()
    await expect(page.getByTestId('seeNextPageBtn')).toBeEnabled()
    await expect(page.getByTestId('seeLastPageBtn')).toBeEnabled()
