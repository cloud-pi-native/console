...
test.describe('Administration Roles', () => {
  test('Should list admin roles', { tag: ['@e2e', '@need-rework'] }, async ({ page }) => {
    // Arrange
    ...
    // @TODO: Do better.
    await expect(page.getByTestId('76229c96-4716-45bc-99da-00498ec9018c-tab')).toBeVisible();
    await expect(page.getByTestId('eadf604f-5f54-4744-bdfb-4793d2271e9b-tab')).toBeVisible();
  });
  ...
  test('Should add a new OIDC role', { tag: '@e2e' }, async ({ page }) => {
    ...
    await expect(page.getByTestId('role-list')).not.toContainText(newOidcRole.name);
    ...
    // Assert
    const membershipCheckbox = page.getByTestId(`input-checkbox-${newOidcRole.user.id}-cbx`);
  ...
  test('Should add a user to a role', { tag: '@e2e' }, async ({ page }) => {
    ...
    await page.getByTestId('test-members').click();
    await expect(page.getByTestId('addUserBtn')).toBeDisabled();
    await page.getByTestId('addUserSuggestionInput').locator('input').fill(`${newRole.user.email}`);
    await page.getByTestId('addUserBtn').click();

    // Assert
    const membershipCheckbox = page.getByTestId(`input-checkbox-${newRole.user.id}-cbx`);
    await expect(membershipCheckbox).toBeVisible();
    await expect(membershipCheckbox).toBeChecked();
  });
