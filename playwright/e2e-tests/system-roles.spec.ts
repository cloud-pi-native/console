...
test.describe('System Roles at Project Creation', () => {
  test('Should have built-in system roles after project creation', { tag: '@e2e' }, async ({ page }) => {
    ...
    // Assert
    const systemRoles = [
      { name: 'Administrateur', oidcGroup: '/console/admin', roleType: 'Managé' },
      { name: 'DevOps', oidcGroup: '/console/devops', roleType: 'Managé' },
      { name: 'Développeur', oidcGroup: '/console/developer', roleType: 'Managé' },
      { name: 'Lecture seule', oidcGroup: '/console/readonly', roleType: 'Managé' },
    ]
