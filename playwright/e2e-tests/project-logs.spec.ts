...
test.describe('Project logs page', () => {
  test('Should display project logs as owner', { tag: '@e2e' }, async ({ page }) => {
    // Arrange
    ...
    await expect(page.getByTestId('positionInfo')).toContainText('1 - 5 sur 5');
  });
  ...
  test(
    ...
    async ({ page }) => {
      ...
      // Assert
      await expect(page.locator('#panel-logs')).toBeVisible();
      await expect(page.getByTestId('positionInfo')).toContainText('1 - 5 sur 6');
    },
  );
