  ...
  test(
    ...
    async ({ page }) => {
      ...
      try {
        ...
        await expect(page.getByTestId('teamTable').getByText(secondTestUser.email)).toBeVisible({ timeout: 300_000 })
        ...
        const roleMemberRow = page.getByRole('row').filter({ hasText: secondTestUser.email }).first()
        ...
        const memberRow = gitlabPage.getByRole('row').filter({ hasText: `@${secondTestUser.username}` }).first()
