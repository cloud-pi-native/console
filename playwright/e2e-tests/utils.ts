...
const clusterSyncApiRegex = /\/api\/.*\/clusters(?:\/[^/?#]+)?(?:\?.*)?$/
...
export function waitForClusterSync({
  page,
  timeoutMs = 60000,
}: {
  page: Page
  ...
  return page.waitForResponse((response) => {
    ...
    return (method === 'POST' || method === 'PUT')
      && success
      && clusterSyncApiRegex.test(response.url())
  }, { timeout: timeoutMs })
}
...
export async function createCluster({
  ...
  if (!(await page.getByTestId('menuAdministrationClusters').isVisible()))
    await page.getByTestId('menuAdministrationBtn').click()
  ...
  await Promise.all([
    ...
    page.getByTestId('addClusterBtn').click(),
  ])
  await expect(page.getByTestId('cpin-loader')).toHaveCount(0)
...
export async function waitForCluster({
  ...
  await expect(page.getByTestId(`clusterLink-${name}`)).toBeVisible({ timeout: timeoutMs })
}
