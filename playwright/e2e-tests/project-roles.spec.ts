...
test.describe.serial('Project roles', { tag: '@e2e' }, () => {
  ...
  test('Should kebab-case oidc group on save', async ({ page }) => {
    ...
    await page.getByTestId('roleOidcGroupInput').fill('/custom-group')
    await page.getByTestId('roleNameInput').fill('Another Name')
    await expect(page.getByTestId('roleOidcGroupInput')).toHaveValue('/custom-group')
    ...
    expect(body[0]?.oidcGroup).toBe('/custom-group')
