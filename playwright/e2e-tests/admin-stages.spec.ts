...
test.describe('Stages administration page', { tag: '@e2e' }, () => {
  ...
  test('should update a custom stage', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText(
      `Informations du type d\'environnement ${stageName}`,
    );
  ...
  test('should not be able to create a stage with an existing name', async ({ page }) => {
    ...
    await expect(
      page.getByTestId('snackbar').getByText("Un type d'environnement portant ce nom existe déjà"),
    ).toBeVisible();
  });
  ...
  test('should not delete a used custom stage', async ({ page }) => {
    ...
    await expect(page.getByTestId('associatedEnvironmentsZone')).toContainText(
      "Le type d'environnement ne peut être supprimé",
    );
