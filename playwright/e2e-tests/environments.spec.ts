...
test.describe('Environments page', { tag: '@e2e' }, () => {
  ...
  test('should not add environments to a project without enough hprod GPU', async ({ page }) => {
    ...
    await expect(
      page
        ...
        .getByText('Le projet ne dispose pas de suffisamment de ressources : GPU.'),
    ).toBeVisible();
  });
  ...
  test('should not add incorrect environments', async ({ page }) => {
    ...
    await expect(
      page.getByTestId('snackbar').getByText("Ce nom d'environnement est déjà pris"),
    ).toBeVisible();
  });
  ...
  test('should not update environment resources when cluster is too small', async ({ page }) => {
    ...
    await expect(
      page
        .getByTestId('snackbar')
        .getByText('Le cluster ne dispose pas de suffisamment de ressources : Mémoire.'),
    ).toBeVisible();
  });
  ...
  test('should show a warning if autosync is deactivated', async ({ page }) => {
    await page.goto(clientURL);
    await signInCloudPiNative({ page, credentials: testUser });
    await createProject({ page });
    const envName = await createEnvironment({
    ...
    await expect(page.getByTestId('noAutosyncAlert')).toHaveText(
      'La synchronisation automatique est désactivée. Les déploiements devront être synchronisés manuellement.',
    );
  });
});
