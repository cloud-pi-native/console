...
test.describe('Integration tests user flow: first checks', { tag: '@integ' }, () => {
  ...
  test('Pipelines run', { tag: '@replayable' }, async ({ page }) => {
    ...
    await expect(page1.getByRole('link', { name: 'Status: Passed test-vault' })).toBeVisible();
    await expect(page1.getByRole('link', { name: 'Status: Passed test-nexus' })).toBeVisible();
    await expect(page1.getByRole('link', { name: 'Status: Passed test-harbor' })).toBeVisible();
    await expect(page1.getByRole('link', { name: 'Status: Passed test-sonar' })).toBeVisible();
  });
});
...
test.describe('Integration tests user flow: deployment and metrics', { tag: '@integ' }, () => {
  ...
  test('ArgoCD deployment', { tag: '@replayable' }, async ({ page }) => {
    ...
    await page1
      .getByText('app.kubernetes.io/managed-by=dso-console, dso/environment=integ, dso/')
      .click();
...
test.describe('Integration tests user flow: Cleanup', { tag: '@integ' }, () => {
  ...
  test('Remove stage', async ({ page }) => {
    ...
    await expect(page.getByRole('cell', { name: 'Aucun environnement existant' })).toBeVisible();
    ...
    const page1 = await page1Promise;
    await page1.getByRole('button', { name: 'Log in via Keycloak' }).click();
    await expect(page1.getByRole('combobox', { name: 'Search applications...' })).toBeVisible();
    await expect(
