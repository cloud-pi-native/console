  ...
  test(
    ...
    async ({ page }) => {
      ...
      try {
        ...
        await gitlabPage.locator('.gl-spinner').first().waitFor({ state: 'hidden', timeout: projectReadinessTimeoutMs })
  ...
  test(
    ...
    async ({ page }) => {
      ...
      try {
        ...
        await expect(page.getByTestId('snackbar')).toContainText('Rôle mis à jour', { timeout: projectReadinessTimeoutMs })
        ...
        await expect(memberRow).toContainText(developerRoleRegexp, { timeout: projectReadinessTimeoutMs })
      } finally {
        await deleteProject({ page, projectName })
      }
    },
