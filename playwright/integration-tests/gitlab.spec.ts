...
import { projectReadinessTimeoutMs } from '../helpers/constants';
    ...
    async ({ page }) => {
      ...
      try {
        ...
        await gitlabPage
          .locator('.gl-spinner')
          .first()
          .waitFor({ state: 'hidden', timeout: projectReadinessTimeoutMs });

        await expect(
          gitlabPage.getByTestId('group-name').filter({ hasText: 'infra-apps' }),
        ).toBeVisible({ timeout: projectReadinessTimeoutMs });
        await expect(
        ...
        const manageLink = gitlabPage
          ...
          .first();
        await manageLink.click({ timeout: projectReadinessTimeoutMs });
        await gitlabPage
          .locator('.gl-spinner')
          .first()
          .waitFor({ state: 'hidden', timeout: projectReadinessTimeoutMs });
        ...
        await membersLink.click({ timeout: projectReadinessTimeoutMs });
        await gitlabPage
          .locator('.gl-spinner')
          .first()
          .waitFor({ state: 'hidden', timeout: projectReadinessTimeoutMs });
        ...
        const userRow = gitlabPage
          .getByRole('row')
          .filter({ hasText: `@${currentUserGitlabUsername}` })
          .first();
        await userRow.scrollIntoViewIfNeeded();
        await expect(userRow).toBeVisible({
          timeout: projectReadinessTimeoutMs,
        });

        const botRow = gitlabPage
          .getByRole('row')
          .filter({ hasText: `${projectSlug}-bot` })
          .first();
  ...
  test(
    ...
    async ({ page }) => {
      ...
      try {
        ...
        await page
          .locator('input[data-testid="addUserSuggestionInput"]')
          .fill(secondTestUser.email);
        await page.getByTestId('addUserBtn').click();
        const teamMemberRow = page
          .getByTestId('teamTable')
          .getByRole('row')
          .filter({ hasText: secondTestUser.email })
          .first();
        await expect(teamMemberRow).toBeVisible({ timeout: projectReadinessTimeoutMs });
        const teamMemberUserId = (
          (await teamMemberRow.locator('code').first().textContent()) ?? ''
        ).trim();
        expect(teamMemberUserId).not.toEqual('');

        await page.reload();
        await expect(page.getByTestId('project-slug')).toBeVisible({
          timeout: projectReadinessTimeoutMs,
        });

        await page.getByTestId('test-tab-roles').click();
        await page.locator('[data-testid$="-tab"]').filter({ hasText: 'DevOps' }).click();
        await page.getByRole('tab', { name: 'Membres' }).click();
        const roleMemberCheckbox = page.getByTestId(`input-checkbox-${teamMemberUserId}-cbx`);
        await expect(roleMemberCheckbox).toBeVisible({ timeout: projectReadinessTimeoutMs });
        ...
        const memberRow = gitlabPage
          .getByRole('row')
          .filter({ hasText: `@${secondTestUserGitlabUsername}` })
          .first();
        await expect(memberRow).toContainText(developerRoleRegexp, {
          timeout: projectReadinessTimeoutMs,
        });
      } finally {
        await deleteProject({ page, projectName });
