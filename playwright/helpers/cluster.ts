...
async function selectClusterConfidentiality(page: Page, confidentiality: ClusterConfidentiality) {
  await page.getByLabel('Confidentialité du cluster').selectOption({ label: confidentiality });
...
async function addStageToClusterAssociationByName(page: Page, stageName: string) {
  await page.getByTestId('choice-selector-search-stages-select').fill(stageName);
...
async function addStagesToClusterAssociationByNames(page: Page, stageNames: string[]) {
  for (const stageName of stageNames) await addStageToClusterAssociationByName(page, stageName);
}
...
export async function createCluster({
  ...
  if (zone) await selectClusterZone(page, zone);
  ...
  if (informations) await fillClusterInformations(page, informations);
