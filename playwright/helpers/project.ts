...
async function fillProjectResources({
  ...
  const fillHprod = async (resources: Resources) => {
    ...
    await page.getByTestId('memoryHprodInput').fill(resources.memory.toString());
  };
  const fillProd = async (resources: Resources) => {
    ...
    await page.getByTestId('memoryProdInput').fill(resources.memory.toString());
  };

  if (hprodResources) await fillHprod(hprodResources);
  if (prodResources) await fillProd(prodResources);
}
...
async function addMembersToProject(page: Page, members: Credentials[]) {
  ...
  for (const member of members) {
    ...
    await expect(page.getByTestId('teamTable').getByText(member.email)).toBeVisible();
  }
...
export function projectSlugTextRegexp(projectSlug: string): RegExp {
  const escapedSlug = RegExp.escape(projectSlug);
  return new RegExp(`^slug:\\s*${escapedSlug}$`);
}
...
export async function createProject({
  ...
  if (members?.length) await addMembersToProject(page, members);
