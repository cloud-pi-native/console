...
import { deleteValidationInput, uiLongActionTimeoutMs } from './constants'
...
export async function addProject({
  ...
  await expect(page).toHaveURL(projectUrlRegexp, { timeout: uiLongActionTimeoutMs })
  await expect(page.getByTestId('project-slug')).toBeVisible({ timeout: uiLongActionTimeoutMs })
  if (members?.length)
