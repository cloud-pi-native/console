...
async function fillRepositoryUrl(page: Page, externalRepoUrlInput?: string) {
  const url =
    externalRepoUrlInput ?? `${faker.internet.url({ appendSlash: true })}myrepository.git`;
...
export async function createRepository({
  ...
  if (infraRepo) await page.getByText("Dépôt contenant du code d'").click();
  await page.getByTestId('addRepoBtn').click();
  await expect(page.getByTestId(`repoTr-${repositoryName}`)).toContainText(repositoryName);
