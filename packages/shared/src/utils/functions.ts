...
export const getLongestStringOfArray = (array: Array<string>) =>
  array.reduce((acc, curr) => (acc.length < curr.length ? curr : acc), '');

export function calcProjectNameMaxLength() {
  return 61 - longestEnvironmentName;
}

export const getUniqueListBy = (arr: Array<Record<string, unknown>>, key: string) => [
...
function excludeCircular(
  ...
  if (typeof value === 'object') {
    if (inPath.has(value)) return '[object Circular]';
    inPath.add(value);
...
export function resourceListToDictByKey<T extends { key: string }>(
  resList: Array<T>,
): ResourceByKey<T> {
  return resList.reduce((acc, curr) => {
    return {
 ...
 *
 */
export function shallowMatch(
  objectController: Record<string, unknown> | undefined,
  objectToCheck: Record<string, unknown> | undefined,
) {
