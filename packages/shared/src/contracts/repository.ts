...
export const repositoryContract = contractInstance.router(
  {
    ...
    listRepositories: {
      ...
      responses: {
        200: z.array(
          RepoSchema.omit({ internalRepoName: true }).extend({ internalRepoName: z.string() }),
        ),
        500: ErrorSchema,
      },
    },
...
export type CreateRepositoryBody = ClientInferRequest<
  typeof repositoryContract.createRepository
>['body'];

export type UpdateRepositoryBody = ClientInferRequest<
  typeof repositoryContract.updateRepository
>['body'];

export type SyncRepositoryParams = ClientInferRequest<
  typeof repositoryContract.syncRepository
>['params'];

export type RepositoryParams = ClientInferRequest<
  typeof repositoryContract.updateRepository
>['params'];
