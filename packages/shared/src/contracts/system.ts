...
import { pluginSchema, pluginUpdateBody, SystemSettingSchema } from '../schemas/index.js';
...
export const systemSettingsContract = contractInstance.router(
  {
    listSystemSettings: {
      ...
      query: SystemSettingSchema.pick({ key: true }).partial(),
      responses: {
...
export type SystemSettings = ClientInferResponseBody<
  typeof systemSettingsContract.listSystemSettings,
  200
>;

export type SystemSetting = SystemSettings[number];

export type UpsertSystemSettingBody = ClientInferRequest<
  typeof systemSettingsContract.upsertSystemSetting
>['body'];
