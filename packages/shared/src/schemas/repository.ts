...
export const RepoFormSchema = RepoSchema.omit({ createdAt: true, updatedAt: true }).extend({
  isStandalone: z.boolean(),
});

export const UpdateRepoFormSchema = RepoFormSchema.refine(
...
export const CreateRepoFormSchema = RepoFormSchema.omit({ id: true, projectId: true })
  .refine(
    ({ isPrivate, externalToken, externalUserName }) => {
      if (isPrivate) {
        if (!externalToken && !externalUserName) return false;
