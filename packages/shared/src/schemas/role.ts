...
export const AdminRoleSchema = RoleSchema.extend({
  oidcGroup: z.string(),
  type: z.string().optional().default('managed'),
});

export const RoleNameCsvSchema = z
  .string()
  .refine((value) => {
...
export type RoleBigint = Omit<Zod.infer<typeof RoleSchema>, 'permissions'> & {
  permissions: bigint;
};
export type AdminRole = Zod.infer<typeof AdminRoleSchema>;
export type ProjectRole = Zod.infer<typeof ProjectRoleSchema>;
export type ProjectRoleBigint = Omit<Zod.infer<typeof ProjectRoleSchema>, 'permissions'> & {
  permissions: bigint;
};
