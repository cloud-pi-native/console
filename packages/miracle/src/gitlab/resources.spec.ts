...
describe('gitlab resources', () => {
  ...
  it('gitlabMirrorCreds create: creates token', async () => {
    const provider = PROVIDERS.get('gitlab:MirrorCreds')
    if (!provider) throw new Error('Missing provider')

    const client = {
      validateTokenForGroup: vi.fn(),
      groupAccessTokensCreate: vi.fn().mockResolvedValue({ name: 'bot', token: 'tok' }),
    }

    const ctx = createCtx('create')
    await provider.handler.call(ctx, 'mirror-creds-x', { client, groupId: 1, tokenName: 't' })

    expect(client.groupAccessTokensCreate).toHaveBeenCalledWith(1, 't', ['write_repository', 'read_repository', 'read_api'], expect.any(String))
    expect(ctx.create).toHaveBeenCalledWith({ MIRROR_USER: 'bot', MIRROR_TOKEN: 'tok', groupId: 1 })
  })

  it('gitlabMirrorCreds update: reuses when token is valid and not rotating', async () => {
    const provider = PROVIDERS.get('gitlab:MirrorCreds')
    if (!provider) throw new Error('Missing provider')
    ...
    const ctx = createCtx('update', { MIRROR_USER: 'bot', MIRROR_TOKEN: 'tok', groupId: 1 })
    await provider.handler.call(ctx, 'mirror-creds-x', { client, groupId: 1, tokenName: 't' })
  ...
  it('gitlabMirrorTriggerToken create: creates trigger token', async () => {
    const provider = PROVIDERS.get('gitlab:MirrorTriggerToken')
    if (!provider) throw new Error('Missing provider')

    const client = {
      pipelineTriggerTokensCreate: vi.fn().mockResolvedValue({ token: 'tok' }),
    }

    const ctx = createCtx('create')
    await provider.handler.call(ctx, 'mirror-trigger-x', { client, repoId: 123, description: 'd' })

    expect(ctx.create).toHaveBeenCalledWith({ token: 'tok', repoId: 123 })
  })

  it('gitlabMirrorTriggerToken update: reuses when not rotating and repo matches', async () => {
    const provider = PROVIDERS.get('gitlab:MirrorTriggerToken')
    if (!provider) throw new Error('Missing provider')
    ...
    const ctx = createCtx('update', { token: 'tok', repoId: 123 })
    await provider.handler.call(ctx, 'mirror-trigger-x', { client, repoId: 123, description: 'd' })
