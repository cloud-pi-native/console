...
export const TimeRotation = Resource('time:Rotation', async function (
  ...
  if (!previous) {
    return this.create({ tick: props.tick, rotatedAt: props.tick, generation: 1, rotated: true })
  }

  const shouldRotate = diffDays(props.tick, previous.rotatedAt) >= props.rotateEveryDays
  if (shouldRotate) {
    return this.create({ tick: props.tick, rotatedAt: props.tick, generation: previous.generation + 1, rotated: true })
  }
  return this.create({ tick: props.tick, rotatedAt: previous.rotatedAt, generation: previous.generation, rotated: false })
})
