      ...
      for (const [hook, functions] of objectEntries(subscribedHooks)) {
        ...
        for (const [step, fn] of objectEntries(functions?.steps ?? {})) {
          ...
          if (hook === 'checkServices' && step !== 'check') {
            logger.warn(
              { plugin: name, hook, step },
              'Tried to register an invalid step for checkServices hook',
            );
            continue;
          }

          if (
            'uniquePlugin' in hooks[hook] &&
            hooks[hook]?.uniquePlugin !== '' &&
            hooks[hook]?.uniquePlugin !== name
          ) {
            logger.warn(
              { plugin: name, hook, registeredBy: hooks[hook].uniquePlugin },
              'Hook is already registered by another plugin',
            );
            continue;
          }
          // @ts-ignore
