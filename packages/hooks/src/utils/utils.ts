...
export type DeclareModuleGenerator<
  Infos extends ServiceInfos & Pick<Required<ServiceInfos>, 'config'>,
...
export const enabledOrDefaultOrNullish = (value?: string): boolean | undefined =>
  value ? [ENABLED, DEFAULT].includes(value) : true;
export const disabledOrDefaultOrNullish = (value?: string): boolean | undefined =>
  value ? [DISABLED, DEFAULT].includes(value) : true;
export const specificallyDisabled = (value?: string): boolean | undefined =>
  value ? value === DISABLED : undefined;
export const specificallyEnabled = (value?: string): boolean | undefined =>
  value ? value === ENABLED : undefined;
export const defaultOrNullish = (value?: string): boolean | undefined =>
...
export function uniqueResource<T extends { metadata?: { name?: string } }>(...lists: T[][]): T[] {
  return lists
    ...
    .reduce(
      (acc, cur) =>
        acc.some((item) => item.metadata?.name === cur.metadata?.name) ? acc : [...acc, cur],
      [] as T[],
    );
}
