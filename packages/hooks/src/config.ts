...
export function addPlugin(
  ...
  editStrippersObject.global = editStrippersObject.global.merge(
    z.object({ [pluginName]: editZod.global.default({}) }),
  );
  editStrippersObject.project.admin = editStrippersObject.project.admin.merge(
    z.object({ [pluginName]: editZod.project.admin.default({}) }),
  );
  editStrippersObject.project.user = editStrippersObject.project.user.merge(
    z.object({ [pluginName]: editZod.project.user.default({}) }),
  );
}
...
export function populatePluginManifests({
  data,
  select,
  permissionTarget,
  pluginName,
}: PopulateManifestsParams): Partial<PluginConfig> {
  ...
  for (const [scope] of objectEntries(select).filter(([_scope, bool]) => bool)) {
    ...
    selected[scope] = manifest[scope]
      .filter(
        (item) =>
          item.permissions[permissionTarget].read || item.permissions[permissionTarget].write,
      )
      .map((item) => {
        const row = data[scope]?.find(
          (candidate) => candidate.pluginName === pluginName && candidate.key === item.key,
        );
        if (item.kind === 'switch') {
