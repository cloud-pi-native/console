...
export class RegistryService {
  ...
  private async ensureRetentionPolicy(projectSlug: string, harborProjectId: number) {
    const span = trace.getActiveSpan()
    span?.setAttributes({
      'project.slug': projectSlug,
  ...
  async ensureProject(project: ProjectWithDetails, options: { storageLimitBytes?: number, publishProjectRobot?: boolean } = {}) {
    ...
    await Promise.all([
      ...
      Number.isFinite(harborProjectId) ? this.ensureRetentionPolicy(project.slug, harborProjectId) : Promise.resolve(),
