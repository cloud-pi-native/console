...
export class RegistryService {
  ...
  private async ensureRobot(projectSlug: string, robotName: string, access: HarborAccess[]): Promise<VaultRobotSecret> {
    ...
    const vaultRobotSecretResult = await this.vault.read(vaultPath)
    if (vaultRobotSecretResult.error && vaultRobotSecretResult.error.kind !== 'NotFound') {
      throw new Error(`Failed to read registry robot secret from Vault: ${vaultRobotSecretResult.error.kind}`)
    }
    const vaultRobotSecret = vaultRobotSecretResult.error?.kind === 'NotFound'
      ? null
      : vaultRobotSecretResult.data

    if (vaultRobotSecret?.data?.HOST === this.harborHost) {
    ...
    const written = await this.vault.write(secret, vaultPath)
    if (written.error) {
      throw new Error(`Failed to write registry robot secret to Vault: ${written.error.kind}`)
    }
    return secret
  }
