...
export class RegistryService {
  ...
  private async getOrCreateProjectQuota(projectSlug: string, storageLimit: number) {
    const span = trace.getActiveSpan()
    span?.setAttributes({
      'project.slug': projectSlug,
  ...
  async ensureProject(projectSlug: string, options: { storageLimitBytes?: number, publishProjectRobot?: boolean } = {}) {
    ...
    const harborProject = await this.getOrCreateProjectQuota(projectSlug, storageLimit)
