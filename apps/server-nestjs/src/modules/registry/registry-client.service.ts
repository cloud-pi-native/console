...
export class RegistryClientService {
  ...
  async getProjectByName(projectName: string) {
    return this.fetch(`/projects/${encodeURIComponent(projectName)}`, {
      method: 'GET',
  ...
  async listQuotas(projectId: number) {
    return this.fetch(`/quotas?reference_id=${encodeURIComponent(String(projectId))}`, {
      method: 'GET',
    })
  }

  async updateQuota(projectId: number, storageLimit: number) {
    return this.fetch(`/quotas/${encodeURIComponent(String(projectId))}`, {
      method: 'PUT',
      body: {
        hard: {
  ...
  async getGroupMembers(projectName: string) {
    return this.fetch(`/projects/${encodeURIComponent(projectName)}/members`, {
      method: 'GET',
      headers: { 'X-Is-Resource-Name': 'true' },
    })
  }
  ...
  async removeGroupMember(projectName: string, memberId: number) {
    return this.fetch(`/projects/${encodeURIComponent(projectName)}/members/${encodeURIComponent(String(memberId))}`, {
      method: 'DELETE',
      headers: { 'X-Is-Resource-Name': 'true' },
    })
  }
  ...
  async getProjectRobots(projectName: string) {
    return this.fetch(`/projects/${encodeURIComponent(projectName)}/robots`, {
      method: 'GET',
      headers: { 'X-Is-Resource-Name': 'true' },
    })
  }
  ...
  async deleteRobot(projectName: string, robotId: number) {
    const direct = await this.fetch(`/robots/${encodeURIComponent(String(robotId))}`, {
      method: 'DELETE',
    })
    if (direct.status < 300 || direct.status === 404) return direct

    return this.fetch(`/projects/${encodeURIComponent(projectName)}/robots/${encodeURIComponent(String(robotId))}`, {
      method: 'DELETE',
      headers: { 'X-Is-Resource-Name': 'true' },
    })
  }
  ...
  async updateRetention(retentionId: number, body: any) {
    return this.fetch(`/retentions/${encodeURIComponent(String(retentionId))}`, {
      method: 'PUT',
      body,
    })
  }
  ...
  private async getRobot(projectSlug: string, robotName: string): Promise<HarborRobot | undefined> {
    ...
    const robots = await this.getProjectRobots(projectSlug)
  ...
  private async createProjectRobot(projectSlug: string, robotName: string, access: HarborAccess[]): Promise<HarborRobotCreated> {
    const created = await this.createRobot(
  ...
  async addProjectGroupMember(projectSlug: string, groupName: string, accessLevel: number = 3) {
    ...
    const members = await this.getGroupMembers(projectSlug)
    ...
    if (existing?.id) {
      if (existing.role_id !== accessLevel && existing.entity_type !== 'g') {
        await this.removeGroupMember(projectSlug, Number(existing.id))
      } else {
    ...
    const created = await this.addGroupMember(projectSlug, {
      role_id: accessLevel,
      member_group: {
  ...
  private async upsertProject(projectSlug: string, storageLimit: number): Promise<HarborProject> {
    const span = trace.getActiveSpan()
    span?.setAttributes({
      'project.slug': projectSlug,
  ...
  async provisionProject(projectSlug: string, options: { storageLimitBytes?: number, publishProjectRobot?: boolean } = {}) {
    ...
    const project = await this.upsertProject(projectSlug, storageLimit)
    const projectId = Number(project.project_id)
