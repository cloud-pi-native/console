...
export class RegistryClientService {
  ...
  async listQuotas(projectId: number) {
    return this.http.fetch<HarborProjectQuota[]>(`/api/v2.0/quotas?reference_id=${encodeURIComponent(String(projectId))}`, {
      method: 'GET',
    })
  }
  ...
  async getGroupMembers(projectName: string) {
    return this.http.fetch<HarborMember[]>(`/api/v2.0/projects/${encodeURIComponent(projectName)}/members`, {
      method: 'GET',
      headers: { 'X-Is-Resource-Name': 'true' },
    })
  }
  ...
  async getProjectRobots(projectName: string) {
    return this.http.fetch<HarborRobot[]>(`/api/v2.0/projects/${encodeURIComponent(projectName)}/robots`, {
      method: 'GET',
      headers: { 'X-Is-Resource-Name': 'true' },
    })
  }
  ...
  async createRobot(body: HarborRobotCreateRequest) {
    return this.http.fetch<HarborRobotCreated>('/api/v2.0/robots', {
      method: 'POST',
      body,
    })
  }
