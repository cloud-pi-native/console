...
describe('projectService', () => {
  ...
  describe('create', () => {
    it('generates slug, creates project, emits event, returns ProjectV2', async () => {
      ...
      const result = await service.create(body, userId, requestId)
    ...
    it('throws InternalServerErrorException when project cannot be loaded after creation', async () => {
      ...
      await expect(service.create(body, userId, faker.string.uuid()))
  ...
  describe('list', () => {
    ...
    it('forbids "all" filter for non-admin users', async () => {
      ...
      await expect(
        service.listProjects(makeListProjectsQuery({ filter: 'all' }), user),
  ...
  describe('update', () => {
    it('updates description and returns updated project', async () => {
      ...
      const tx = mockDeep<Prisma.TransactionClient>()
      tx.project.findFirst.mockResolvedValue(pwd)
