...
describe('projectService', () => {
  ...
  describe('createProject', () => {
    ...
    it('throws UnprocessableEntityException when hook emission fails', async () => {
      const userId = faker.string.uuid()
      const body = makeCreateProjectBody()

      const tx = mockDeep<Prisma.TransactionClient>()
      tx.project.findUnique.mockResolvedValue(makeProjectWithDetails() as never)
      prisma.$transaction.mockImplementation(async cb => cb(tx))
      events.emitAsync.mockRejectedValue(new Error('hook failed'))
  ...
  describe('updateProject', () => {
    ...
    it('validates new owner is a human account', async () => {
      ...
      tx.project.findUniqueOrThrow.mockResolvedValue({
        ...pwd,
        members: [makeProjectMemberWithUser(makeUser({ id: newOwnerId, type: 'bot' }))],
      } as never)
      prisma.$transaction.mockImplementation(async cb => cb(tx))
    ...
    it('transfers ownership correctly', async () => {
      ...
      tx.project.findUniqueOrThrow.mockResolvedValue({
        ...pwd,
        members: [makeProjectMemberWithUser(
  ...
  describe('archiveProject', () => {
    ...
    it('throws UnprocessableEntityException when hook emission fails', async () => {
      ...
      await expect(service.archiveProject(projectId))
        .rejects.toThrow(UnprocessableEntityException)
    })
  })
  ...
  describe('replayHooksForProject', () => {
    ...
    it('throws UnprocessableEntityException when hook emission fails', async () => {
      ...
      await expect(service.replayHooksForProject(projectId))
        .rejects.toThrow(UnprocessableEntityException)
    })
  })
  ...
  describe('getProjectsData', () => {
    it('returns CSV string for admin', async () => {
      ...
      const result = await service.getProjectsData(adminPerms)
    ...
    it('throws ForbiddenException for non-admin', async () => {
      await expect(service.getProjectsData(0n)).rejects.toThrow(ForbiddenException)
    })
  })
  ...
  describe('bulkActionProject', () => {
    it('processes specific project ids', async () => {
      ...
      await service.bulkActionProject(data, userId, adminPerms)
    ...
    it('resolves "all" to all non-archived project ids', async () => {
      ...
      await service.bulkActionProject({ action: 'archive', projectIds: 'all' }, userId, adminPerms)
    ...
    it('throws ForbiddenException for non-admin', async () => {
      ...
      await expect(
        service.bulkActionProject({ action: 'archive', projectIds: 'all' }, userId, 0n),
      ).rejects.toThrow(ForbiddenException)
    })
