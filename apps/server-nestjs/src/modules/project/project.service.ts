...
export class ProjectService {
  ...
  async list(
    ...
    if (filter === 'all' && !AdminAuthorized.Manage(user.adminPermissions)) {
      this.logger.warn(`project.list forbidden (requestorUserId=${user.id}, filter=${filter})`)
    ...
    this.logger.debug(`project.list started (requestorUserId=${user.id}, filter=${filter})`)
    const projects = await listProjects(this.prisma, whereAnd)
    span?.setAttribute('project.list.count', projects.length)
    this.logger.debug(`project.list completed (requestorUserId=${user.id}, filter=${filter}, count=${projects.length})`)
  ...
  async update(
    ...
    span?.setAttribute('project.id', projectId)
    span?.setAttribute('user.id', user.id)
    this.logger.log(`project.update started (projectId=${projectId}, requestorUserId=${user.id})`)
    try {
      const project = await this.prisma.$transaction(async (tx) => {
        const projectDb = await getNotArchivedProjectForUpdate(tx, projectId)
        if (!projectDb) throw new NotFoundException()

        const isOwner = projectDb.ownerId === user.id
        ...
        if (strippedKeys.length) {
          ...
          this.logger.debug(`project.update strippedFields (projectId=${projectId}, requestorUserId=${user.id}, strippedKeys=${strippedKeys.join(',')})`)
        }
        ...
        if (ownerIdCandidate && ownerIdCandidate !== projectDb.ownerId) {
          this.logger.log(`project.update ownerChange started (projectId=${projectId}, requestorUserId=${user.id}, previousOwnerId=${projectDb.ownerId}, nextOwnerId=${ownerIdCandidate})`)
          ...
          this.logger.log(`project.update ownerChange completed (projectId=${projectId}, requestorUserId=${user.id}, previousOwnerId=${projectDb.ownerId}, nextOwnerId=${ownerIdCandidate})`)
        }
        ...
        this.logger.log(`project.update dbUpdated (projectId=${projectId}, requestorUserId=${user.id}, effectiveKeys=${effectiveKeys.join(',')})`)
        return updated
      })
      await this.eventEmitter.emitAsync('project.upsert', project)
      span?.setAttribute('project.slug', project.slug)
      this.logger.log(`project.update completed (projectId=${projectId}, requestorUserId=${user.id})`)
      return project
    } catch (error) {
      this.logger.error(
        `project.update failed (projectId=${projectId}, requestorUserId=${user.id}): ${error instanceof Error ? error.message : String(error)}`,
        error instanceof Error ? error.stack : undefined,
      )
      throw error
    }
