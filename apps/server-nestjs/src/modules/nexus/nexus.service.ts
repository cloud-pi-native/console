...
export class NexusService {
  ...
  private async ensureProjectGroupRoles(project: ProjectWithDetails, args: { readOnlyPrivileges: string[], writePrivileges: string[] }) {
    const rawWriteSuffixes = await this.getOptionalConfigValue(project, PROJECT_WRITE_GROUP_PATH_SUFFIXES_PLUGIN_KEY)
      ?? [
        await this.getEffectiveConfigValue(project, PROJECT_ADMIN_GROUP_PATH_SUFFIX_PLUGIN_KEY, DEFAULT_PROJECT_ADMIN_GROUP_PATH_SUFFIX),
        await this.getEffectiveConfigValue(project, PROJECT_DEVELOPER_GROUP_PATH_SUFFIX_PLUGIN_KEY, DEFAULT_PROJECT_DEVELOPER_GROUP_PATH_SUFFIX),
        await this.getEffectiveConfigValue(project, PROJECT_DEVOPS_GROUP_PATH_SUFFIX_PLUGIN_KEY, DEFAULT_PROJECT_DEVOPS_GROUP_PATH_SUFFIX),
      ].join(',')

    const rawReadSuffixes = await this.getOptionalConfigValue(project, PROJECT_READ_GROUP_PATH_SUFFIXES_PLUGIN_KEY)
      ?? [
        await this.getEffectiveConfigValue(project, PROJECT_READONLY_GROUP_PATH_SUFFIX_PLUGIN_KEY, DEFAULT_PROJECT_READONLY_GROUP_PATH_SUFFIX),
        await this.getEffectiveConfigValue(project, PROJECT_SECURITY_GROUP_PATH_SUFFIX_PLUGIN_KEY, DEFAULT_PROJECT_SECURITY_GROUP_PATH_SUFFIX),
      ].join(',')
