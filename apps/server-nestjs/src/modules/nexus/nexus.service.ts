...
export class NexusService {
  ...
  private async ensureProject(project: ProjectWithDetails) {
    ...
    const password = await this.ensureUser(project, existingPassword)
  ...
  private async ensureUser(project: ProjectWithDetails, password: string | undefined) {
    ...
    if (existing) {
      if (!password || password !== ensuredPassword) {
