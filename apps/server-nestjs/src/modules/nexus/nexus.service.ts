...
function generateRolePrivilegesMapping(args: { readGroupPaths: string[], writeGroupPaths: string[], readOnlyPrivileges: string[], writePrivileges: string[] }) {
  ...
  for (const groupPath of args.readGroupPaths) byId.set(toNexusRoleIdFromGroupPath(groupPath), args.readOnlyPrivileges)
  for (const groupPath of args.writeGroupPaths) byId.set(toNexusRoleIdFromGroupPath(groupPath), args.writePrivileges)
  return byId
}
