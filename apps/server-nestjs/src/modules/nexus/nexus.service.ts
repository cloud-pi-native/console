...
export class NexusService {
  ...
  async provisionProject(args: {
    ...
    const vaultSecretResult = await this.vault.read(vaultPath)
    if (vaultSecretResult.error && vaultSecretResult.error.kind !== 'NotFound') {
      throw new Error(`Failed to read Nexus secret from Vault: ${vaultSecretResult.error.kind}`)
    }
    const existingPassword = vaultSecretResult.error
      ? undefined
      : vaultSecretResult.data.data?.NEXUS_PASSWORD
    ...
    if (written.error) {
      throw new Error(`Failed to write Nexus secret to Vault: ${written.error.kind}`)
    }
  }
  ...
  async deleteProject(projectSlug: string) {
    ...
    const vaultPath = getProjectVaultPath(this.config.projectRootPath, projectSlug, 'tech/NEXUS')
    const destroyed = await this.vault.destroy(vaultPath)
    if (destroyed.error && destroyed.error.kind !== 'NotFound') {
      throw new Error(`Failed to destroy Nexus secret in Vault: ${destroyed.error.kind}`)
    }
  }
