...
export class NexusService {
  ...
  private async upsertRepo<TBody>(options: {
    ...
    const existing = await this.client.request(options.getUrl, {
      method: 'GET',
      validateStatus: code => [200, 404].includes(code),
    })
    if (existing.status === 404) {
      await this.client.request(options.createUrl, {
        method: 'POST',
        body: options.body,
    ...
    await this.client.request(options.updateUrl, {
      method: 'PUT',
      body: options.body,
  ...
  private async upsertPrivilege(body: { name: string, description: string, actions: string[], format: string, repository: string }) {
    const existing = await this.client.request(`/security/privileges/${encodeURIComponent(body.name)}`, {
      method: 'GET',
      validateStatus: code => [200, 404].includes(code),
    })
    if (existing.status === 404) {
      await this.client.request('/security/privileges/repository-view', {
        method: 'POST',
        body,
    ...
    await this.client.request(`/security/privileges/repository-view/${encodeURIComponent(body.name)}`, {
      method: 'PUT',
      body,
  ...
  private async createMavenRepos(projectSlug: string, options: { snapshotWritePolicy: WritePolicy, releaseWritePolicy: WritePolicy }) {
    ...
    await this.client.request('/repositories/maven/group', {
      method: 'POST',
      body: {
  ...
  private async upsertRole(projectSlug: string, privileges: string[]) {
    ...
    const role = await this.client.request(`security/roles/${encodeURIComponent(roleId)}`, {
      method: 'GET',
      validateStatus: code => [200, 404].includes(code),
    })
    if (role.status === 404) {
      await this.client.request('/security/roles', {
        method: 'POST',
        body: {
          id: roleId,
    ...
    await this.client.request(`security/roles/${encodeURIComponent(roleId)}`, {
      method: 'PUT',
      body: {
        id: roleId,
        name: `${projectSlug}-role`,
  ...
  private async ensureUser(projectSlug: string, ownerEmail: string, password: string) {
    const getUser = await this.client.request<{ userId: string }[]>(`/security/users?userId=${encodeURIComponent(projectSlug)}`)

    const existing = (getUser.data ?? []).find(u => u.userId === projectSlug)
    if (existing) {
      await this.client.request(`/security/users/${encodeURIComponent(projectSlug)}/change-password`, {
        method: 'PUT',
        body: password,
        headers: {
    ...
    await this.client.request('/security/users', {
      method: 'POST',
      body: {
        userId: projectSlug,
        firstName: 'Monkey D.',
  ...
  async deleteProject(projectSlug: string) {
    ...
    await Promise.all([
      ...
      this.client.request(`/security/users/${encodeURIComponent(projectSlug)}`, {
        method: 'DELETE',
