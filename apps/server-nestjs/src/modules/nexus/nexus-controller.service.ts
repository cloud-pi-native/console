...
export class NexusControllerService {
  ...
  private async createMavenRepos(projectSlug: string, options: { snapshotWritePolicy: string, releaseWritePolicy: string }) {
    ...
    const hostedRepos = [
      { repoName: names.hosted[0].repo, writePolicy: options.releaseWritePolicy },
      { repoName: names.hosted[1].repo, writePolicy: options.snapshotWritePolicy },
    ]

    await Promise.all(hostedRepos.map(async ({ repoName, writePolicy }) => {
