...
describe('nexusService', () => {
  ...
  it('handleCron should reconcile all projects', async () => {
    const projects = [
      makeProjectWithDetails({ slug: 'project-1', plugins: [{ key: NEXUS_CONFIG_KEY_ACTIVATE_MAVEN_REPO, value: ENABLED }] }),
      makeProjectWithDetails({ slug: 'project-2', plugins: [{ key: NEXUS_CONFIG_KEY_ACTIVATE_NPM_REPO, value: ENABLED }] }),
    ]
  ...
  it('dedupes project group roles by role id and keeps the last privileges', async () => {
    const project = makeProjectWithDetails({
      slug: 'project-1',
      owner: { email: 'owner@example.com', firstName: 'Owner', lastName: 'User' },
      plugins: [
    ...
    expect(client.createSecurityRoles).toHaveBeenCalledWith(expect.objectContaining({
      id: 'kc-project-1-console-devops',
      privileges: expect.arrayContaining(['project-1-privilege-group-ro']),
    }))
  })
})
