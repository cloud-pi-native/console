...
export class NexusClientService {
  ...
  private get baseUrl() {
    ...
    const base = this.config.nexusInternalUrl.endsWith('/')
      ? this.config.nexusInternalUrl
      : `${this.config.nexusInternalUrl}/`
    return new URL('service/rest/v1/', base).toString()
  }
  ...
  async getSecurityUsers(userId: string): Promise<{ userId: string }[]> {
    const res = await this.fetch<{ userId: string }[]>(`/security/users?userId=${encodeURIComponent(userId)}`, { method: 'GET' })
    return (res.data as any) ?? []
  }

  @StartActiveSpan()
  async putSecurityUsersChangePassword(userId: string, password: string): Promise<void> {
  ...
  @StartActiveSpan()
  async postSecurityUsers(body: any): Promise<void> {
