...
export function makeExpandedUserSchema(params: { id: number, email: string, username: string, name: string }): ExpandedUserSchema {
...
export function makeGitbeakerRequestError(params: { message?: string, status?: number, statusText?: string, description: string }) {
  const request = new Request('http://gitlab.example/api')
