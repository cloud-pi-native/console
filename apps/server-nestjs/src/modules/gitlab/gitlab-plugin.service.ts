...
export class GitlabPluginService {
  ...
  infos(): ServiceInfos {
    return {
      name: 'gitlab',
      to: ({ project }) => {
        if (!this.gitlabConfig.url || !this.gitlabConfig.projectRootDir) return undefined
        return new URL(`${this.gitlabConfig.projectRootDir}/${project.slug}`, this.gitlabConfig.url).toString()
      },
      title: 'Gitlab',
