...
describe('gitlabService', () => {
  ...
  describe('handleUpsert', () => {
    ...
    it('should bind builtin roles (admin/auditor) when role ids are resolved', async () => {
      ...
      expect(gitlab.upsertUser).toHaveBeenCalledWith(expect.objectContaining({ email: 'admin@example.com', isAdmin: true, isAuditor: false }))
      expect(gitlab.upsertUser).toHaveBeenCalledWith(expect.objectContaining({ email: 'auditor@example.com', isAdmin: false, isAuditor: true }))
      expect(gitlab.upsertUser).toHaveBeenCalledWith(expect.objectContaining({ email: 'owner@example.com', isAdmin: true, isAuditor: false }))
    })
