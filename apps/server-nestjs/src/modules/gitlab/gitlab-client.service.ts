...
export type CreateUserOptionsWith<T extends keyof CreateUserOptions> = With<CreateUserOptions, T>
...
export class GitlabClientService {
  ...
  async createUser(user: CreateUserOptions): Promise<UserSchema> {
  ...
  async upsertUser(
    user: Omit<CreateUserOptionsWith<'email' | 'username' | 'name'>, 'externUid' | 'provider'>,
    ...
    const editOptions: EditUserOptions = createOptions as unknown as EditUserOptions

    const gitlabUser: UserSchema = existing ?? await this.createUser(createOptions)
