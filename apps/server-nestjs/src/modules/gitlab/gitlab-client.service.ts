...
export type GroupSchemaWith<T extends keyof GroupSchema> = GroupSchema & Required<Pick<GroupSchema, T>>
...
export class GitlabClientService {
  ...
  async createSubGroup(parentGroup: GroupSchemaWith<'id'>, name: string) {
  ...
  async getOrCreateProjectGroup() {
    ...
    return this.getOrCreateGroupByPath(this.config.projectRootPath)
  }
  ...
  async getOrCreateMirrorPipelineTriggerToken(projectSlug: string): Promise<PipelineTriggerTokenSchema> {
    ...
    return (currentTriggerToken ?? await this.client.PipelineTriggerTokens.create(mirrorRepo.id, TOKEN_DESCRIPTION)) as any
  }
}
