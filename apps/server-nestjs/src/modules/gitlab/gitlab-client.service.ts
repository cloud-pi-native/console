...
export class GitlabClientService {
  ...
  async upsertGroupCustomAttribute(groupId: number, key: string, value: string): Promise<void> {
    ...
    try {
      await (this.client as any).GroupCustomAttributes.set(groupId, key, value)
    } catch (error) {
  ...
  async upsertProjectCustomAttribute(projectId: number, key: string, value: string): Promise<void> {
    ...
    try {
      await (this.client as any).ProjectCustomAttributes.set(projectId, key, value)
    } catch (error) {
  ...
  async upsertUserCustomAttribute(userId: number, key: string, value: string): Promise<void> {
    ...
    try {
      await (this.client as any).UserCustomAttributes.set(userId, key, value)
    } catch (error) {
  ...
  async getOrCreateGroupByPath(path: string) {
    ...
    let currentFullPath = rootGroupPath
    let parentGroup = await this.getGroupByPath(rootGroupPath) ?? await this.createGroup(rootGroupPath)
    const getFullPath = (group: CondensedGroupSchema, fallback: string) => typeof (group as any).full_path === 'string' ? (group as any).full_path as string : fallback

    for (const part of parts) {
      currentFullPath = `${getFullPath(parentGroup, currentFullPath)}/${part}`
  ...
  async getProjectToken(projectSlug: string) {
    ...
    return find(
      this.offsetPaginate<{ name: string }>(opts => this.client.GroupAccessTokens.all(group.id, opts) as any),
  ...
  async getOrCreateMirrorPipelineTriggerToken(projectSlug: string): Promise<PipelineTriggerTokenSchema> {
    ...
    const currentTriggerToken = await find(
      this.offsetPaginate<PipelineTriggerTokenSchema>(opts => this.client.PipelineTriggerTokens.all(mirrorRepo.id, opts) as any),
  ...
  private async* offsetPaginate<T>(
    ...
    while (page !== null) {
      ...
      try {
        ...
        this.logger.debug(`Pagination page fetched (page=${page}, nextPage=${nextPage === null ? 'null' : nextPage}, items=${data.length}, total=${total})`)
