...
export class GitlabService implements OnModuleInit {
  ...
  private pendingCommits: Record<number, {
  ...
  public async commitCreateOrUpdate(
    ...
    this.addActions(repoId, branch, message, [{
      action,
      filePath,
      content,
    }])
  ...
  public async commitDelete(repoId: number, paths: string[]): Promise<void> {
    ...
    this.addActions(repoId, branch, 'ci: :robot_face: Delete files', actions)
  }

  public async commitFiles(): Promise<number> {
    let filesUpdated = 0
    for (const [idStr, repo] of Object.entries(this.pendingCommits)) {
