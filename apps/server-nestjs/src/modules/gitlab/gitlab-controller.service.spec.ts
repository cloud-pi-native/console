...
describe('gitlabControllerService', () => {
  ...
  describe('handleUpsert', () => {
    it('should reconcile project members and repositories', async () => {
      ...
      gitlabService.getOrCreateProjectGroup.mockResolvedValue(group as any)
      gitlabService.getGroupMembers.mockResolvedValue([])
      gitlabService.listRepositories.mockResolvedValue([])
      gitlabService.getProjectToken.mockResolvedValue({ token: 'token' } as any)
      gitlabService.createEmptyProjectRepository.mockResolvedValue({ id: 1 } as any)
      ...
      gitlabService.findUserByEmail.mockResolvedValue({ id: 123, username: 'user' } as any)
      // @ts-ignore
