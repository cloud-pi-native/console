    ...
    providers: [
      ...
      },
      {
        ...
        useValue: {
          ...
          readProjectMirrorCreds: vi.fn(),
          writeProjectMirrorCreds: vi.fn(),
...
function makeVaultSecret<T extends Record<string, unknown>>(data: T) {
  return {
    data,
    metadata: {
...
describe('gitlabControllerService', () => {
  ...
  describe('handleUpsert', () => {
    it('should reconcile project members and repositories', async () => {
      ...
      vault.read.mockResolvedValue(makeVaultSecret({ MIRROR_TOKEN: 'token' }))
    ...
    it('should create gitlab user if not exists', async () => {
      ...
      expect(gitlab.editGroupMember).toHaveBeenCalledWith(group.id, 999, AccessLevel.NO_ACCESS)
    ...
    it('should configure repository mirroring if external url is present', async () => {
      ...
      gitlab.createProjectMirrorAccessToken.mockResolvedValue(accessToken)
      vault.readProjectMirrorCreds.mockResolvedValue(null)
      gitlab.upsertProjectMirrorRepo.mockResolvedValue(makeProjectSchema({ id: 1, name: 'mirror', path: 'mirror', path_with_namespace: 'forge/console/project-1/mirror', empty_repo: false }))
      gitlab.getOrCreateMirrorPipelineTriggerToken.mockResolvedValue(makePipelineTriggerToken())

      await service.handleUpsert(project)

      expect(gitlab.createProjectMirrorAccessToken).toHaveBeenCalledWith('project-1')
      ...
      expect(vault.writeProjectMirrorCreds).toHaveBeenCalledWith('project-1', {
        MIRROR_USER: 'bot',
