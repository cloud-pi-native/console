...
export class GitlabControllerService {
  ...
  private async purgeOrphanMembers(groupId: number, validGitlabUserIds: SimpleUserSchema[], members: MemberSchema[]) {
    for (const member of members) {
      ...
      if (!validGitlabUserIds.find(u => u.id === member.id)) {
        if (this.configService.gitlabControllerPurgeOrphanMembers) {
  ...
  private async ensureRepos(project: ProjectWithDetails) {
    const gitlabRepositories = await getAll(this.gitlabService.getRepos(project.slug))
  ...
  private async ensureRepository(project: ProjectWithDetails, repo: ProjectWithDetails['repositories'][number], gitlabRepositories: ProjectSchema[]) {
    ...
    if (!gitlabRepo) {
      gitlabRepo = await this.gitlabService.upsertProjectGroupRepo(
        project.slug,
  ...
  private async configureRepositoryMirroring(
    ...
    const internalRepoUrl = await this.gitlabService.getProjectGroupInternalRepoUrl(project.slug, repo.internalRepoName)
  ...
  private async readGitlabMirrorCreds(projectSlug: string, repoName: string) {
    const vaultCredsPath = `${this.configService.projectRootPath}/${projectSlug}/${repoName}-mirror`
    return this.vaultService.read(vaultCredsPath)
  }

  private async writeGitlabMirrorCreds(projectSlug: string, repoName: string, data: Record<string, any>) {
    const vaultCredsPath = `${this.configService.projectRootPath}/${projectSlug}/${repoName}-mirror`
    return this.vaultService.write(data, vaultCredsPath)
  }

  private async deleteGitlabMirrorCreds(projectSlug: string, repoName: string) {
    const vaultCredsPath = `${this.configService.projectRootPath}/${projectSlug}/${repoName}-mirror`
    return this.vaultService.destroy(vaultCredsPath)
  }
  ...
  private async ensureInfraAppsRepo(projectSlug: string) {
    await this.gitlabService.upsertProjectGroupRepo(projectSlug, INFRA_APPS_REPO_NAME, undefined)
  }

  private async ensureMirrorRepo(projectSlug: string) {
    const mirrorRepo = await this.gitlabService.upsertProjectMirrorRepo(projectSlug)
    if (mirrorRepo.empty_repo) {
      await this.gitlabService.commitMirror(mirrorRepo.id)
    }
    await this.ensureMirrorRepoTriggerToken(projectSlug)
  }

  private async ensureMirrorRepoTriggerToken(projectSlug: string) {
    const triggerToken = await this.gitlabService.getOrCreateMirrorPipelineTriggerToken(projectSlug)
    const gitlabSecret = {
  ...
  private async createProjectMirrorAccessToken(projectSlug: string) {
    const token = await this.gitlabService.createProjectMirrorAccessToken(projectSlug)
    const creds = {
  ...
  private isProjectMirrorCredsExpiring(vaultSecret: VaultSecret): boolean {
    ...
    return daysAgoFromNow(createdTime) > this.configService.gitlabMirrorTokenRotationThresholdDays
  }
}
