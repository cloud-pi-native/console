...
import type { MemberSchema, ProjectSchema, SimpleUserSchema } from '@gitbeaker/core'
...
@Injectable()
export class GitlabControllerService implements OnApplicationBootstrap {
  ...
  private getPluginConfig(project: ProjectWithDetails, pluginName: string, key: string) {
  ...
  private async ensureMembers(project: ProjectWithDetails) {
    ...
    await this.purgeOrphanMembers(group.id, validGitlabUsers, members, project)
  }
  ...
  private async purgeOrphanMembers(
    groupId: number,
    validGitlabUserIds: SimpleUserSchema[],
    members: MemberSchema[],
    project: ProjectWithDetails,
  ) {
    const purgeConfig = this.getPluginConfig(project, 'gitlab', 'purge')

    for (const member of members) {
      if (this.isManagedUser(member)) continue
      if (!validGitlabUserIds.some(u => u.id === member.id)) {
