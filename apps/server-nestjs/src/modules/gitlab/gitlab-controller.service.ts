...
export class GitlabControllerService {
  ...
  private async upsertUser(user: ProjectWithDetails['members'][number]['user']) {
    ...
    if (!gitlabUser) {
      gitlabUser = await this.gitlab.createUser(
        user.email,
  ...
  private async ensureProjectRepos(project: ProjectWithDetails) {
    ...
    for (const repo of project.repositories) {
      ...
      if (repo.externalRepoUrl) {
        ...
        const deleted = await this.vault.deleteGitlabMirrorCreds(project.slug, repo.internalRepoName)
        if (deleted.error) {
          throw new Error(`Failed to delete GitLab mirror creds: ${deleted.error.kind}`)
        }
      }
    }
  }
  ...
  private async configureRepositoryMirroring(
    ...
    const currentVaultSecretResult = await this.vault.readGitlabMirrorCreds(project.slug, repo.internalRepoName)
    if (currentVaultSecretResult.error && currentVaultSecretResult.error.kind !== 'NotFound') {
      throw new Error(`Failed to read GitLab mirror creds: ${currentVaultSecretResult.error.kind}`)
    }
    const currentVaultSecret = currentVaultSecretResult.error?.kind === 'NotFound'
      ? null
      : currentVaultSecretResult.data
    ...
    const written = await this.vault.writeGitlabMirrorCreds(project.slug, repo.internalRepoName, mirrorSecretData)
    if (written.error) {
      throw new Error(`Failed to write GitLab mirror creds: ${written.error.kind}`)
    }
  }
  ...
  private async ensureMirrorRepoTriggerToken(projectSlug: string) {
    ...
    const written = await this.vault.writeMirrorTriggerToken(gitlabSecret)
    if (written.error) {
      throw new Error(`Failed to write mirror trigger token: ${written.error.kind}`)
    }
  }

  private async getOrRotateMirrorCreds(projectSlug: string) {
    const vaultSecretResult = await this.vault.readMirrorCreds(projectSlug)
    if (vaultSecretResult.error) {
      if (vaultSecretResult.error.kind === 'NotFound') return this.createMirrorAccessToken(projectSlug)
      throw new Error(`Failed to read mirror creds: ${vaultSecretResult.error.kind}`)
    }

    if (!this.isMirrorCredsExpiring(vaultSecretResult.data)) {
      return vaultSecretResult.data.data as { MIRROR_USER: string, MIRROR_TOKEN: string }
    }
  ...
  private async createMirrorAccessToken(projectSlug: string) {
    ...
    const written = await this.vault.writeMirrorCreds(projectSlug, creds)
    if (written.error) {
      throw new Error(`Failed to write mirror creds: ${written.error.kind}`)
    }
    return creds
  }
