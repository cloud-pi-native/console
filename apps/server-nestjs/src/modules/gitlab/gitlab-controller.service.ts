...
export class GitlabControllerService {
  ...
  private async configureRepositoryMirroring(
    ...
    let currentVaultSecret: VaultSecret | null = null
    try {
      currentVaultSecret = await this.vault.readGitlabMirrorCreds(project.slug, repo.internalRepoName)
    } catch (error) {
      if (!(error instanceof VaultError && error.kind === 'NotFound')) throw error
    }
  ...
  private async getOrRotateMirrorCreds(projectSlug: string) {
    ...
    try {
      vaultSecret = await this.vault.readMirrorCreds(projectSlug)
    } catch (error) {
      if (error instanceof VaultError && error.kind === 'NotFound') return this.createMirrorAccessToken(projectSlug)
      throw error
    }
  ...
  private async createMirrorAccessToken(projectSlug: string) {
    ...
    await this.vault.writeMirrorCreds(projectSlug, creds)
    return creds
  }
