...
import {
  makeProjectServicesCluster,
  makeProjectServicesEnvironment,
  makeProjectServicesPlugin,
  makeProjectServicesProject,
  makeProjectServicesProjectPlugin,
  makeProjectServicesUpdateBody,
  makeProjectServicesZone,
...
describe('projectServicesService', () => {
  ...
  let project: ReturnType<typeof makeProjectServicesProject>
  let cluster: ReturnType<typeof makeProjectServicesCluster>
  let plugin: ReturnType<typeof makeProjectServicesPlugin>
  ...
  beforeEach(async () => {
    ...
    plugin = makeProjectServicesPlugin({
      title: serviceTitle,
    ...
    project = makeProjectServicesProject({ slug: projectSlug })
    cluster = makeProjectServicesCluster()
  ...
  it('get returns the service with actual hooks data', async () => {
    ...
    prisma.projectPlugin.findMany.mockResolvedValue([
      makeProjectServicesProjectPlugin({ pluginName, value: ENABLED }),
    ])
  ...
  it('get includes zones coming from project environments', async () => {
    ...
    const projectWithEnvironmentZones = makeProjectServicesProject({
      clusters: [],
      environments: [
        makeProjectServicesEnvironment({
          cluster: makeProjectServicesCluster({
            zone: makeProjectServicesZone(environmentZone),
          }),
        }),
      ],
    })
    const envZonePlugin = makeProjectServicesPlugin({
  ...
  it('update stores project configuration through the real utility', async () => {
    const data = makeProjectServicesUpdateBody(pluginName)
