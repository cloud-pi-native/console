    ...
    return tracer.startActiveSpan('fetch', async (span) => {
      try {
        ...
        if (response.status === 404) {
          throw new VaultError('NotFound', response.statusText ?? 'Not Found', {
            status: 404,
            method,
            path,
        ...
        if (!response.ok) {
          throw new VaultError('HttpError', response.statusText, {
        ...
        try {
          ...
          throw new VaultError('ParseError', error instanceof Error ? error.message : String(error), { method, path })
        }
      } catch (error) {
