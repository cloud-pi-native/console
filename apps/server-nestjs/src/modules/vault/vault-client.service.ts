...
export class VaultClientService {
  ...
  async getKvData<T = any>(kvName: string, path: string): Promise<VaultSecret<T>> {
    ...
    span?.setAttribute('vault.kv.path', normalizedPath)
    const response = await this.fetch<VaultResponse<T>>(`/v1/${kvName}/data/${normalizedPath}`, { method: 'GET' })
    if (!response?.data) {
      throw new VaultError('InvalidResponse', 'Missing "data" field', { method: 'GET', path: `/v1/${kvName}/data/${normalizedPath}` })
    }
  ...
  async upsertKvData<T = any>(kvName: string, path: string, body: { data: T }): Promise<void> {
    ...
    span?.setAttribute('vault.kv.name', kvName)
    span?.setAttribute('vault.kv.path', normalizedPath)
    await this.fetch(`/v1/${kvName}/data/${normalizedPath}`, { method: 'POST', body })
  }

  @StartActiveSpan()
  async destroy(path: string): Promise<void> {
    const span = trace.getActiveSpan()
    span?.setAttribute('vault.kv.path', normalizeKvPath(path))
  ...
  async deleteKvMetadata(kvName: string, path: string): Promise<void> {
    ...
    span?.setAttribute('vault.kv.path', normalizedPath)
    try {
      await this.fetch(`/v1/${kvName}/metadata/${normalizedPath}`, { method: 'DELETE' })
    } catch (error) {
  ...
  async listKvMetadata(kvName: string, path: string): Promise<string[]> {
    try {
      ...
      span?.setAttribute('vault.kv.name', kvName)
      span?.setAttribute('vault.kv.path', normalizedPath)
      const response = await this.fetch<VaultListResponse>(`/v1/${kvName}/metadata/${normalizedPath}`, { method: 'LIST' })
      if (!response?.data?.keys) {
        throw new VaultError('InvalidResponse', 'Missing "data.keys" field', { method: 'LIST', path: `/v1/${kvName}/metadata/${normalizedPath}` })
      }
