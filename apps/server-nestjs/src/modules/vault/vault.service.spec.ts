...
describe('vault', () => {
  ...
  describe('getProjectValues', () => {
    it('should get project values', async () => {
      ...
      expect(result).toEqual({ data: { secret: 'value' }, error: null })
    })
  ...
  describe('read', () => {
    it('should read secret', async () => {
      ...
      expect(result).toEqual({
        data: { data: { secret: 'value' }, metadata: { created_time: '2023-01-01T00:00:00.000Z', version: 1 } },
        error: null,
      })
    })
  ...
  describe('write', () => {
    it('should write secret', async () => {
      const result = await service.write({ secret: 'value' }, 'path')
  ...
  describe('destroy', () => {
    it('should destroy secret', async () => {
      const result = await service.destroy('path')
      expect(result).toEqual({ data: undefined, error: null })
    })
  })
})
