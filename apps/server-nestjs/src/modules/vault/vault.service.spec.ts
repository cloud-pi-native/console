...
import { describe, beforeEach, it, expect, beforeAll, afterAll, afterEach } from 'vitest'
...
describe('vault', () => {
  ...
  describe('write', () => {
    ...
    it('should expose response body on error', async () => {
      server.use(
        http.post(`${vaultInternalUrl}/v1/kv/data/:path`, () => {
      ...
      await expect(service.write({ secret: 'value' }, 'path')).rejects.toMatchObject({
        kind: 'HttpError',
        status: 400,
        responseBody: { errors: ['No secret engine mount at test-project/'] },
      })
      await expect(service.write({ secret: 'value' }, 'path')).rejects.toThrow('No secret engine mount at test-project/')
    })
  })

  describe('destroy', () => {
    it('should destroy secret', async () => {
      await expect(service.destroy('path')).resolves.toBeUndefined()
    })
  })
})
