...
export class VaultService {
  ...
  async read(path: string): Promise<VaultSecret> {
    trace.getActiveSpan()?.setAttribute('vault.path', path)
  ...
  async writeMirrorTriggerToken(secret: Record<string, any>) {
    trace.getActiveSpan()?.setAttribute('vault.path', 'GITLAB')
  ...
  async upsertZone(zoneName: string): Promise<void> {
    trace.getActiveSpan()?.setAttributes({
      'zone.name': zoneName,
      'vault.kvName': generateZoneName(zoneName),
    })
  ...
  async upsertProject(project: ProjectWithDetails): Promise<void> {
    trace.getActiveSpan()?.setAttribute('project.slug', project.slug)
  ...
  async deleteProject(projectSlug: string): Promise<void> {
    trace.getActiveSpan()?.setAttribute('project.slug', projectSlug)
  ...
  private async ensureProjectGroup(groupName: string, policyName: string): Promise<void> {
    trace.getActiveSpan()?.setAttributes({
      'vault.groupName': groupName,
      'vault.policyName': policyName,
    })
    ...
    try {
      trace.getActiveSpan()?.setAttributes({
        'vault.groupAliasName': groupAliasName,
        'vault.oidcAccessor': oidc.accessor,
      })
