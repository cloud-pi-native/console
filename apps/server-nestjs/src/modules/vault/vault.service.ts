...
export class VaultService {
  ...
  private async upsertMount(kvName: string): Promise<void> {
    ...
    try {
      await this.vaultClientService.createSysMount(kvName, body)
    } catch (error) {
      if (error instanceof VaultError && error.kind === 'HttpError' && error.status === 400) {
        await this.vaultClientService.tuneSysMount(kvName, body)
        return
      }
      throw error
    }
    ...
    return tracer.startActiveSpan('upsertProject', async (span) => {
      ...
      try {
        ...
        await Promise.all([
          ...
          this.vaultClientService.upsertAuthApproleRole(roleName, this.getApproleRoleBody([techPolicyName, appPolicyName])),
        ])
      } catch (error) {
        if (error instanceof Error) span.recordException(error)
        throw error
  ...
  async createTechnicalReadOnlyPolicy(name: string, projectSlug: string): Promise<void> {
