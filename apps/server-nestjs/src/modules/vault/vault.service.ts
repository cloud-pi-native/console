...
export class VaultService {
  ...
  async upsertProject(project: ProjectWithDetails): Promise<void> {
    ...
    await Promise.all([
      ...
      ...projectAdminGroupPaths.map(groupPath =>
        this.ensureIdentityGroup(generateProjectAdminGroupName(project), [appPolicyName], groupPath)),
      ...projectDevopsGroupPaths.map(groupPath =>
        this.ensureIdentityGroup(generateProjectDevopsGroupName(project), [projectDevopsPolicyName], groupPath)),
      ...projectDeveloperGroupPaths.map(groupPath =>
        this.ensureIdentityGroup(generateProjectDeveloperGroupName(project), [projectDeveloperPolicyName], groupPath)),
      ...projectReadOnlyGroupPaths.map(groupPath =>
        this.ensureIdentityGroup(generateProjectReadOnlyGroupName(project), [projectReadOnlyPolicyName], groupPath)),
      ...projectSecurityGroupPaths.map(groupPath =>
        this.ensureIdentityGroup(generateProjectSecurityGroupName(project), [projectSecurityPolicyName], groupPath)),
  ...
  async deleteProject(project: ProjectWithDetails): Promise<void> {
    ...
    const settled = await Promise.allSettled([
      ...
      this.client.deleteIdentityGroupName(generateProjectAdminGroupName(project)),
      this.client.deleteIdentityGroupName(generateProjectDevopsGroupName(project)),
      this.client.deleteIdentityGroupName(generateProjectDeveloperGroupName(project)),
      this.client.deleteIdentityGroupName(generateProjectReadOnlyGroupName(project)),
      this.client.deleteIdentityGroupName(generateProjectSecurityGroupName(project)),
    ])
    for (const result of settled) {
      if (result.status !== 'rejected') continue
      const error = result.reason
...
function generateProjectAdminGroupName(project: ProjectWithDetails) {
  return `project-${project.slug}-admin`
}

function generateProjectDevopsGroupName(project: ProjectWithDetails) {
  return `project-${project.slug}-devops`
}

function generateProjectDeveloperGroupName(project: ProjectWithDetails) {
  return `project-${project.slug}-developer`
}

function generateProjectReadOnlyGroupName(project: ProjectWithDetails) {
  return `project-${project.slug}-readonly`
}

function generateProjectSecurityGroupName(project: ProjectWithDetails) {
  return `project-${project.slug}-security`
}
