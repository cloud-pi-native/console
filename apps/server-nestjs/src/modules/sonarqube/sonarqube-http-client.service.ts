...
export class SonarqubeHttpClientService {
  ...
  private get defaultHeaders(): Record<string, string> {
    if (!this.config.sonarqubeToken) throw new SonarqubeError('NotConfigured', 'SONARQUBE_TOKEN is required')
    return {
      Authorization: `Basic ${Buffer.from(`${this.config.sonarqubeToken}:`, 'utf8').toString('base64')}`,
    }
  }
