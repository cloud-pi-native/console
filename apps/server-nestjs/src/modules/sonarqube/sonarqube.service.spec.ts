...
describe('sonarqubeService', () => {
  ...
  beforeEach(async () => {
    ...
    client.createUserGroups.mockResolvedValue(undefined)
  ...
  describe('handleUpsert', () => {
    it('should create the 5 project role groups in SonarQube', async () => {
      ...
      expect(client.createUserGroups).toHaveBeenCalledWith(expect.objectContaining({ name: `/${project.slug}/console/admin` }))
      expect(client.createUserGroups).toHaveBeenCalledWith(expect.objectContaining({ name: `/${project.slug}/console/devops` }))
      expect(client.createUserGroups).toHaveBeenCalledWith(expect.objectContaining({ name: `/${project.slug}/console/developer` }))
      expect(client.createUserGroups).toHaveBeenCalledWith(expect.objectContaining({ name: `/${project.slug}/console/security` }))
      expect(client.createUserGroups).toHaveBeenCalledWith(expect.objectContaining({ name: `/${project.slug}/console/readonly` }))
    })
    ...
    it('should not recreate user or write vault when both user and secret exist', async () => {
      ...
      vault.readSonarqubeUser.mockResolvedValue({ data: { SONAR_USERNAME: project.slug, SONAR_PASSWORD: 'pw', SONAR_TOKEN: 'tok' }, metadata: {} })
