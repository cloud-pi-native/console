...
export class KeycloakControllerService implements OnModuleInit {
  ...
  private async getOrCreateConsoleGroup(projectGroup: GroupRepresentation) {
    ...
    return this.keycloakService.getOrCreateSubGroupByName(projectGroup.id, this.consoleGroupName)
  }
  ...
  private async ensureEnvironmentGroup(
    ...
    const envGroup = await this.keycloakService.getOrCreateSubGroupByName(consoleGroup.id, environment.name)
    if (!envGroup.id) {
    ...
    const [roGroup, rwGroup] = await Promise.all([
      this.keycloakService.getOrCreateSubGroupByName(envGroup.id, 'RO'),
      this.keycloakService.getOrCreateSubGroupByName(envGroup.id, 'RW'),
    ])
