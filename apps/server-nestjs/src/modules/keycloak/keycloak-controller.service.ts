...
export class KeycloakControllerService {
  ...
  private async ensureProjectGroup(project: ProjectWithDetails, projectGroup: GroupRepresentationWith<'id' | 'name'>) {
      ...
      if (role.oidcGroup) {
        ...
        switch (role.type) {
          case 'managed':
            ...
            break
          case 'external':
            await this.addMissingRoleMembers(roleGroup, project, role, groupMembers)
            break
          case 'global':
            this.logger.debug(`Skipping global role ${role.id} for project ${project.slug}`)
            break
        }
      }
    }))
  ...
  private async addMissingRoleMembers(
    roleGroup: GroupRepresentationWith<'id' | 'name'>,
    project: ProjectWithDetails,
  ...
  private async deleteExtraRoleMembers(
    roleGroup: GroupRepresentationWith<'id' | 'name' | 'path'>,
    project: ProjectWithDetails,
    role: ProjectWithDetails['roles'][number],
    members: UserRepresentation[],
  ) {
    await Promise.all(members.map(async (member) => {
      if (!isMember(project, member) && member.groups?.includes(roleGroup.path)) {
        if (!member.id) {
  ...
  private async ensureEnvironmentGroups(project: ProjectWithDetails, projectGroup: GroupRepresentationWith<'id'>) {
    const consoleGroup = z.object({
      id: z.string(),
    }).parse(await this.keycloak.getOrCreateConsoleGroup(projectGroup))
    await Promise.all([
  ...
  private async purgeOrphanEnvironmentGroups(consoleGroup: GroupRepresentationWith<'id'>, project: ProjectWithDetails) {
    const promises: Promise<void>[] = []
  ...
  private async ensureEnvironmentGroup(
    consoleGroup: GroupRepresentationWith<'id'>,
    ...
    }).parse(await this.keycloak.getOrCreateEnvironmentGroups(consoleGroup, environment))
