...
import { getAll, isMember, map } from './keycloack.utils'
...
export class KeycloakControllerService {
  ...
  private async ensureRoleGroups(
    ...
    return tracer.startActiveSpan('ensureRoleGroups', async (span) => {
      try {
        span.setAttribute('project.slug', project.slug)
        span.setAttribute('keycloak.group.id', group.id)
        span.setAttribute('project.roles.count', project.roles.length)
        ...
        await Promise.all(project.roles.map(async (role) => {
          ...
          return tracer.startActiveSpan('ensureRoleGroup', async (roleSpan) => {
            try {
              ...
              roleSpan.setAttribute('keycloak.group.id', roleGroup.id)
              roleSpan.setAttribute('keycloak.group.path', roleGroup.path)

              const groupMembers = await this.keycloak.getGroupMembers(roleGroup.id)
              roleSpan.setAttribute('keycloak.group.members.current', groupMembers.length)
              ...
              if (error instanceof Error) {
                roleSpan.recordException(error)
              }
              throw error
            } finally {
              roleSpan.end()
  ...
  private async ensureRoleGroupMembers(
    ...
    return tracer.startActiveSpan('ensureRoleGroupMembers', async (span) => {
      try {
        span.setAttribute('project.slug', project.slug)
  ...
  private async purgeOrphanRoleGroupMembers(
    ...
    return tracer.startActiveSpan('purgeOrphanRoleGroupMembers', async (span) => {
      try {
        span.setAttribute('project.slug', project.slug)
        span.setAttribute('role.id', role.id)
        span.setAttribute('role.type', role.type)
