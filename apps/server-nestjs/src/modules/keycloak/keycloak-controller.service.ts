...
export class KeycloakControllerService implements OnApplicationBootstrap {
  ...
  async handleUpsert(project: ProjectWithDetails) {
    return tracer.startActiveSpan('handleUpsert', async (span) => {
      try {
        ...
        // Only reconcile the specific project that was updated
        // This avoids processing all projects and potential errors with inconsistent state in other projects
        const result = await this.ensureProjectGroups([project])
        span.end()
        return result
      } catch (error) {
  ...
  private async reconcile() {
    return tracer.startActiveSpan('reconcile', async (span) => {
      try {
