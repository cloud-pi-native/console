...
export class KeycloakService {
      ...
      if (current) {
        for await (const subgroup of this.getSubGroups(current.id!)) {
          if (subgroup.name === name) {
            current = z.object({ id: z.string() }).parse(subgroup)
            break
          }
        }
        ...
        current = z.object({ id: z.string() }).parse(candidates.find(g => g.path === `/${name}`) ?? candidates.find(g => g.name === name))
      }
  ...
  async getOrCreateGroupByPath(path: string) {
    ...
    for (const name of parts.values()) {
      if (!current) {
        current = z.object({ id: z.string() }).parse(await this.getGroupByName(name) ?? await this.createGroup(name))
      } else {
        if (!parentId) parentId = current.id
        current = z.object({ id: z.string() }).parse(await this.getOrCreateSubGroupByName(parentId, name))
      }
      parentId = current.id
    }
