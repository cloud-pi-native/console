...
export class KeycloakService {
  constructor(
    @Inject(KeycloakClientService) private readonly keycloack: KeycloakClientService,
  ) {
  }
  ...
  async *getAllGroups() {
    let first = 0
    while (true) {
      const fetched = await this.keycloack.client.groups.find({ first, max: 50, briefRepresentation: false })
  ...
  async getGroupByName(name: string): Promise<GroupRepresentation | undefined> {
    const groups = await this.keycloack.client.groups.find({ search: name })
  ...
  async deleteGroup(id: string): Promise<void> {
    await this.keycloack.client.groups.del({ id })
  }

  async getGroupMembers(groupId: string) {
    return this.keycloack.client.groups.listMembers({ id: groupId })
  }

  async createGroup(name: string) {
    return this.keycloack.client.groups.create({ name })
  }

  async addUserToGroup(userId: string, groupId: string) {
    return this.keycloack.client.users.addToGroup({ id: userId, groupId })
  }

  async removeUserFromGroup(userId: string, groupId: string) {
    return this.keycloack.client.users.delFromGroup({ id: userId, groupId })
  }

  async* getSubGroups(parentId: string) {
    let first = 0
    while (true) {
      const page = await this.keycloack.client.groups.listSubGroups({ parentId, briefRepresentation: false, max: 10, first })
  ...
  async getOrCreateSubGroupByName(parentId: string, name: string) {
    ...
    const createdGroup = await this.keycloack.client.groups.createChildGroup({ id: parentId }, { name })
