...
import type { KeycloakService } from './services/keycloak.service'
...
export class KeycloakReconcilerService implements OnModuleInit {
  ...
  private ensureProjectGroups(projects: ProjectWithDetails[]) {
    return Promise.allSettled(projects.map(project => this.ensureProjectGroup(project)))
  }
    ...
    for await (const group of groups) {
      if (group.name && projectSlugs.has(group.name)) {
        if (this.isOwnedConsoleGroup(group)) {
          if (this.configService.keycloakPurge) {
            if (group.id) {
  ...
  private async ensureProjectGroup(project: ProjectWithDetails) {
    const projectGroup = await this.keycloakService.getOrCreateGroup(project.slug)
    await this.ensureProjectMembers(projectGroup, project)
    await this.ensureEnvironmentGroups(projectGroup, project)
  }

  private async ensureProjectMembers(projectGroup: GroupRepresentation, project: ProjectWithDetails) {
    if (!projectGroup.id) {
    ...
    await this.addMissingProjectMembers(projectGroup, project, groupMembers, promises)
    await this.deleteExtraProjectMembers(projectGroup, project, groupMembers, promises)
  ...
  private async addMissingProjectMembers(
    ...
    groupMembers: Awaited<ReturnType<KeycloakService['listGroupMembers']>>,
    ...
    for (const member of project.members) {
      if (!groupMembers.some(m => m.id === member.user.id)) {
        promises.push(
    ...
    if (!groupMembers.some(m => m.id === project.ownerId)) {
      promises.push(
  ...
  private async getOrCreateConsoleGroup(projectGroup: GroupRepresentation) {
    ...
    return this.keycloakService.getOrCreateChildGroup(projectGroup.id, this.consoleGroupName)
  }
  ...
  private async ensureEnvironmentGroup(
    ...
    const envGroup = await this.keycloakService.getOrCreateChildGroup(consoleGroup.id, environment.name)
    if (!envGroup.id) {
    ...
    const [roGroup, rwGroup] = await Promise.all([
      this.keycloakService.getOrCreateChildGroup(envGroup.id, 'RO'),
      this.keycloakService.getOrCreateChildGroup(envGroup.id, 'RW'),
    ])
  ...
  private async ensureEnvironmentMemberPermissions(
    ...
    roMembers: Awaited<ReturnType<KeycloakService['listGroupMembers']>>,
    rwMembers: Awaited<ReturnType<KeycloakService['listGroupMembers']>>,
  ) {
    if (!roGroup.id || !rwGroup.id) {
      throw new Error(`Failed to create or retrieve RO and RW groups for ${environment.name}`)
    }
