...
export class KeycloakClientService implements OnModuleInit {
  ...
  async ensureGroup(name: string) {
    ...
    if (!group) throw new Error(`Keycloak group "${name}" could not be fetched back after creation`)
    return group
  }
  ...
  async getOrCreateSubGroupByName(parentId: string, name: string) {
    ...
    if (!group) throw new Error(`Keycloak subgroup "${name}" under parentId=${parentId} could not be fetched back after creation`)
    return group
  }
