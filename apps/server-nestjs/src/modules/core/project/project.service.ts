...
import type { ProjectDataExport, ProjectDetails, ProjectUpdateContext } from './project-queries.utils.js'
    ...
    try {
      ...
      await this.logs.addLog({
        ...
        data: {
          ...
          messageResume: `Projet créé: ${project.slug} (${project.roles.length} rôles)`,
          results: {
            projectId: project.id,
            slug: project.slug,
            roleCount: project.roles.length,
      ...
      await Promise.all(project.roles.map(async role => this.logs.addLog({
        ...
        data: {
          ...
          messageResume: `Rôle synchronisé: ${role.name}`,
          results: {
            projectId: project.id,
            roleId: role.id,
          },
    ...
    try {
      ...
      await this.logs.addLog({
        ...
        data: {
          ...
          messageResume: `Projet mis à jour: ${project.slug}`,
          results: {
            projectId: project.id,
            slug: project.slug,
          },
    ...
    try {
      ...
      await this.logs.addLog({
        ...
        data: {
          ...
          messageResume: `Projet archivé: ${project.slug}`,
          results: {
            projectId: project.id,
            slug: project.slug,
          },
