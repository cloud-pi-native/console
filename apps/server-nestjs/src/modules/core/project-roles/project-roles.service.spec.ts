...
import { makeProject, makeProjectMembers, makeProjectRole } from './project-roles.testing.utils.js'
...
describe('projectRolesService', () => {
  ...
  it('lists roles with permissions as strings and stripped oidcGroup', async () => {
    ...
    prisma.projectRole.findMany.mockResolvedValue([makeProjectRole({
      name: 'test',
      permissions: 4n,
      position: 0,
      projectId,
  ...
  it('creates a role and emits project update', async () => {
    ...
    prisma.projectRole.findFirst.mockResolvedValue({ position: 0 })
  ...
  it('rejects incoherent positions', async () => {
    ...
    prisma.projectRole.findMany.mockResolvedValue([
      makeProjectRole({ id: roleId1, name: 'a', permissions: 1n, position: 0, projectId, oidcGroup: '' }),
      makeProjectRole({ id: roleId2, name: 'b', permissions: 1n, position: 1, projectId, oidcGroup: '' }),
    ])
