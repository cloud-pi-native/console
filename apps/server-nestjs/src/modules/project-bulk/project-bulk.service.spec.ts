...
describe('projectBulkService', () => {
  ...
  beforeEach(async () => {
    ...
    module = await Test.createTestingModule({
      providers: [
        ...
        { provide: ProjectHooksService, useValue: projectHooksService },
      ],
    }).compile()
  ...
  it('processes specific project ids', async () => {
    ...
    expect(projectService.archive).toHaveBeenCalledTimes(2)
    expect(projectService.archive).toHaveBeenCalledWith(projectIds[0])
    expect(projectService.archive).toHaveBeenCalledWith(projectIds[1])
  })
  ...
  it('lock action updates locked to true via project hooks', async () => {
    ...
    expect(projectHooksService.updateProjectLocked).toHaveBeenCalledWith(projectId, true)
  })
  ...
  it('unlock action updates locked to false via project hooks', async () => {
    ...
    expect(projectHooksService.updateProjectLocked).toHaveBeenCalledWith(projectId, false)
  })
  ...
  it('replay action triggers hooks', async () => {
    ...
    expect(projectHooksService.replayHooks).toHaveBeenCalledWith(projectId)
  })
})
