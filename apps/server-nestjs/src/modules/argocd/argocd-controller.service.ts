...
export class ArgoCDControllerService {
  ...
  async generateValues(
    ...
    const vaultValuesResult = await this.vault.readProjectValues(project.id)
    if (vaultValuesResult.error) {
      throw new Error(`Failed to read project values from Vault: ${vaultValuesResult.error.kind}`)
    }
    const vaultValues = vaultValuesResult.data
