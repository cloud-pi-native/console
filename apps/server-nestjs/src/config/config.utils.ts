...
export function flag(schema: z.ZodType<string, unknown>) {
  return z
    .preprocess(
...
export function csv(schema: z.ZodType<string, unknown>) {
  return z
    .preprocess(
      val => (typeof val === 'string' ? val : val ?? ''),
      z.string().transform(value => value.split(',').map(part => part.trim()).filter(Boolean)),
    )
    .transform((parts: string[]) => parts.map(part => schema.parse(part)))
}
