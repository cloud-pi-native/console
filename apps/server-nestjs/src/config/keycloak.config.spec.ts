...
describe('keycloakConfig', () => {
  ...
  it('composes url/realmUrl/openidConfigurationUrl and parses csv admin ids', () => {
    vi.stubEnv('KEYCLOAK_DOMAIN', 'kc.internal')
    ...
    expect(keycloakConfigFactory()).toMatchObject({
      ...
      url: 'https://kc.internal',
      realmUrl: 'https://kc.internal/realms/dso',
      openidConfigurationUrl: 'https://kc.internal/realms/dso/.well-known/openid-configuration',
      adminKcUserId: ['a', 'b', 'c'],
  ...
  it('honours an explicit protocol', () => {
    ...
    expect(keycloakConfigFactory().url).toBe('https://kc.internal')
