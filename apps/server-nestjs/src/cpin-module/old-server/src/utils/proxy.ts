...
type Tracker<T extends Record<string, any>> =
    | Record<
          keyof T,
          Record<
              string,
...
type Excludes<T extends Target> =
    | Partial<Record<keyof T, Array<keyof T>>>
    | undefined;
const toTarget = <T extends Target>(target: T) => ({
    tracker: {} as Tracker<T>,
    methods: target,
});
            ...
            return async (...args) => {
                ...
                for (const testExclude of excludes[property] ?? []) {
                    ...
                    if (tracker?.[testExclude]?.[id]?.currentExec) {
                        throw new Error(
                            `${String(testExclude)} in progress on ${id}, can't ${String(property)}`,
                        );
                    }
                }
                    ...
                    if (tracker[property][id].currentExec) {
                        return new Promise((resolve) => {
                            tracker[property][id].currentExec.then(() => {
                                resolve(
                                    tracker[property][id].currentExec ??
                                        methods[property](
                                            id,
                                            tracker[property][id].nextArgs,
                                        ),
