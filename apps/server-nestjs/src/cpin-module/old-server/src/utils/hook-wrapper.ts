...
import {
    archiveProject,
    getAdminPlugin,
    getClusterByIdOrThrow,
    getClusterNamesByZoneId,
    getClustersAssociatedWithProject,
    getHookProjectInfos,
    getHookRepository,
    getProjectStore,
    getZoneByIdOrThrow,
    saveProjectStore,
    updateProjectClusterHistory,
...
async function getProjectPayload(
    ...
    reposCreds?: ReposCreds,
) {
    const [project, store, clusters] = await Promise.all([
...
async function upsertProject(
    ...
    const records: ConfigRecords = Object.entries(results.results).reduce(
        (acc, [pluginName, result]) => {
            if (result.store) {
                return [
                    ...acc,
                    ...Object.entries(result.store).map(([key, value]) => ({
                        pluginName,
                        key,
                        value: String(value),
                    })),
    ...
    const project = await manageProjectStatus(
        projectId,
        results,
        'upsert',
        payload.environments.map((env) => env.clusterId),
    );
    return {
        results,
        project,
...
const project = {
    upsert: async (projectId: Project['id'], reposCreds?: ReposCreds) => {
        ...
        return results.results.failed
            ? upsertProject(projectId, reposCreds)
            : results;
    },
    delete: async (projectId: Project['id']) => {
        ...
        const results = await hooks.deleteProject.execute(
            payload,
            dbToObj(config),
        );
        return {
            results,
            project: await manageProjectStatus(
                projectId,
                results,
                'delete',
                [],
...
async function manageProjectStatus(
    projectId: Project['id'],
...
const cluster = {
    upsert: async (
        ...
        if (cluster.zoneId !== previousZoneId) {
            ...
            previousClusterObject.zone =
                await getZoneByIdOrThrow(previousZoneId);
            previousClusterObject.zone.clusterNames =
                await getClusterNamesByZoneId(previousZoneId);
        ...
        clusterObject.zone.clusterNames = await getClusterNamesByZoneId(
            cluster.zoneId,
        );
    ...
    delete: async (clusterId: Cluster['id']) => {
        ...
        clusterObject.zone.clusterNames = clusterNames.filter(
            (c) => c !== cluster.label,
        );
        const store = dbToObj(await getAdminPlugin());
...
const misc = {
    ...
    syncRepository: async (
        repoId: string,
        {
            syncAllBranches,
            branchName,
        }: { syncAllBranches: boolean; branchName?: string },
    ) => {
...
export const hook = {
    ...
    // @ts-ignore TODO voir comment opti la signature de la fonction
    project: genericProxy(project, {
        upsert: ['delete'],
        delete: ['upsert', 'delete'],
        getSecrets: ['delete'],
    }),
    // @ts-ignore TODO voir comment opti la signature de la fonction
    cluster: genericProxy(cluster, {
        delete: ['upsert', 'delete'],
        upsert: ['delete'],
    }),
    // @ts-ignore TODO voir comment opti la signature de la fonction
...
function formatClusterInfos({
    kubeconfig,
    ...cluster
}: Omit<Cluster, 'updatedAt' | 'createdAt' | 'zoneId' | 'kubeConfigId'> & {
    kubeconfig: Kubeconfig;
    zone: Pick<Zone, 'id' | 'slug' | 'label' | 'argocdUrl'>;
}) {
    return {
...
export function transformToHookProject(
    project: ProjectInfos,
    store: Store,
    reposCreds: ReposCreds = {},
): ProjectPayload {
    const clusters = project.clusters.map((cluster) =>
        formatClusterInfos(cluster),
    );
    ...
    return {
        ...
        environments: project.environments.map(({ stage, ...environment }) => ({
            ...
            permissions: [
                {
                    permissions: { rw: true, ro: true },
                    userId: project.ownerId,
                },
                ...project.members.map((member) => ({
                    userId: member.userId,
                    permissions: {
                        ro: ProjectAuthorized.ListEnvironments({
                            adminPermissions: 0n,
                            projectPermissions: getPermsByUserRoles(
                                member.roleIds,
                                rolesById,
                                project.everyonePerms,
                            ),
                        }),
                        rw: ProjectAuthorized.ManageEnvironments({
                            adminPermissions: 0n,
                            projectPermissions: getPermsByUserRoles(
                                member.roleIds,
                                rolesById,
