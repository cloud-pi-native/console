...
// TODO: Convert to NestJS class and inject ConfigurationService to retrieve `appVersion`
const appVersion =
...
type ProjectUpdate = Partial<
    Pick<Project, 'description' | 'ownerId' | 'everyonePerms' | 'locked'>
>;
...
export async function listProjects({
    ...
    if (lastSuccessProvisionningVersion) {
        if (lastSuccessProvisionningVersion === 'outdated')
            whereAnd.push({
                lastSuccessProvisionningVersion: { not: appVersion },
            });
        else if (lastSuccessProvisionningVersion === 'last')
            whereAnd.push({
                lastSuccessProvisionningVersion: { equals: appVersion },
            });
