...
import { BadRequest400, NotFound404 } from '@old-server/utils/errors';
import { hook } from '@old-server/utils/hook-wrapper';
import type { Project, User } from '@prisma/client';

import { logViaSession } from '../user/business';

export const listMembers = async (projectId: Project['id']) =>
    listMembersQuery(projectId);

export async function addMember(
    projectId: Project['id'],
    user: XOR<{ userId: string }, { email: string }>,
    requestorId: User['id'],
    requestId: string,
    ...
    if (user.userId) {
        userInDb = await prisma.user.findUnique({
            where: { id: user.userId, type: 'human' },
        });
    } else if (user.email) {
        userInDb = await prisma.user.findUnique({
            where: { email: user.email, type: 'human' },
        });
    } else {
        return new BadRequest400(
            'Veuillez spécifiez au moins un userId ou un email',
        );
    }
    if (userInDb) {
        if (userInDb.id === projectOwnerId)
            return new BadRequest400(
                'Le owner ne peut pas être ajouté à cette liste',
            );
    } else if (user.email) {
        ...
        if (hookReply.failed) {
            throw new BadRequest400(
                'Echec de la recherche auprès des services externes',
            );
        }

        const retrievedUser = hookReply.results.keycloak?.user;
        if (!retrievedUser) return new BadRequest400('Utilisateur introuvable');
        const userValidated = UserSchema.pick({
            email: true,
            firstName: true,
            lastName: true,
            id: true,
        }).safeParse(retrievedUser);
        if (!userValidated.success)
            return new BadRequest400(
                "L'utilisateur trouvé ne remplit pas les conditions de vérification",
            );
        const logResults = await logViaSession({
            ...userValidated.data,
            groups: [],
        });
...
export async function patchMembers(
    projectId: Project['id'],
    members: typeof projectMemberContract.patchMembers.body._type,
) {
