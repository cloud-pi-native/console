...
describe('test environment business', () => {
    ...
    describe('createEnvironment', () => {
        ...
        it('should create environment and trigger hook', async () => {
            ...
            hook.project.upsert.mockResolvedValue({
                results: {},
                project: { ...project },
            });
    ...
    describe('checkClusterResources', () => {
        it('should authorize cluster not yet configured', async () => {
            ...
            const result = await checkClusterResources(
                { cpu: 1, gpu: 0, memory: 1 },
                cluster,
            );
            expect(result).toBeInstanceOf(Result);
            expect(result.success).toBeTruthy();
        ...
        it('should refuse cluster without enough space', async () => {
            ...
            expect(result.error).toEqual(
                'Le cluster ne dispose pas de suffisamment de ressources : CPU, Mémoire.',
            );
        });
        it('should refuse cluster without GPU', async () => {
            ...
            const result = await checkClusterResources(
                { cpu: 2, gpu: 1, memory: 2 },
                cluster,
            );
            expect(result).toBeInstanceOf(Result);
            expect(result.success).toBeFalsy();
            expect(result.error).toEqual(
                'Le cluster ne dispose pas de suffisamment de ressources : GPU.',
            );
        });
    });
    ...
    describe('checkProjectResources', () => {
        ...
        it('should authorize prod deployment for project with hprod resource but no prod resources', async () => {
            ...
            const result = await checkProjectResources(
                { cpu: 1, gpu: 0, memory: 1, stageId: prodStage.id },
                project,
            );
            expect(result).toBeInstanceOf(Result);
            expect(result.success).toBeTruthy();
        ...
        it('should refuse hprod deployment for project with hprod resource but no prod resources', async () => {
            ...
            const result = await checkProjectResources(
                { cpu: 20, gpu: 20, memory: 20, stageId: hprodStage.id },
                project,
            );
            expect(result).toBeInstanceOf(Result);
            expect(result.success).toBeFalsy();
            expect(result.error).toEqual(
                'Le projet ne dispose pas de suffisamment de ressources : CPU, GPU, Mémoire.',
            );
        });
        it('should refuse overloading hprod deployment', async () => {
            ...
            const result = await checkProjectResources(
                { cpu: 5, gpu: 6, memory: 5, stageId: hprodStage.id },
                project,
            );
            expect(result).toBeInstanceOf(Result);
            expect(result.success).toBeFalsy();
