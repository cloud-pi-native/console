...
export async function checkEnvironmentCreate(input: {
    ...
    if (sameNameEnvironment)
        errorMessages.push("Ce nom d'environnement est déjà pris.");
...
export async function checkClusterResources(
    input: {
        cpu: Environment['cpu'];
        gpu: Environment['gpu'];
        memory: Environment['memory'];
    ...
    if (unsufficientResource.length > 0) {
        return Result.fail(
            `Le cluster ne dispose pas de suffisamment de ressources : ${unsufficientResource.join(', ')}.`,
        );
    }
    return Result.succeed(true);
}

export async function checkProjectResources(
    input: {
        cpu: Environment['cpu'];
        gpu: Environment['gpu'];
        memory: Environment['memory'];
    ...
    const prodStages = await prisma.stage.findMany({
        select: { id: true },
        where: { name: 'prod' },
    });
    ...
    if (stage?.name === 'prod') {
        ...
        overflowResources = await getOverflowResources({
            request: { cpu: input.cpu, gpu: input.gpu, memory: input.memory },
            limit: {
                cpu: project.hprodCpu,
                gpu: project.hprodGpu,
                memory: project.hprodMemory,
            },
            where: {
                projectId: project.id,
                stageId: {
    ...
    if (overflowResources.length > 0) {
        return Result.fail(
            `Le projet ne dispose pas de suffisamment de ressources : ${overflowResources.join(', ')}.`,
        );
    }
    return Result.succeed(true);
}
...
export async function checkEnvironmentUpdate(input: {
    ...
    const projectCheckResult = await checkProjectResources(
        { stageId: environment.stageId, ...input },
        project,
    );
    if (projectCheckResult.isError) {
        errorMessages.push(projectCheckResult.error);
...
export async function getOverflowResources({
    request,
    limit,
    where,
}: {
    ...
    if (
        (environmentResources._sum.memory ?? 0) + request.memory >
        limit.memory
    ) {
