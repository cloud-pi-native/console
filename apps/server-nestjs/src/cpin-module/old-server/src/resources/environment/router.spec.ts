...
const businessCheckEnvironmentCreateMock = vi.spyOn(
    business,
    'checkEnvironmentCreate',
);
const businessCheckEnvironmentUpdateMock = vi.spyOn(
    business,
...
describe('environmentRouter tests', () => {
    ...
    describe('listEnvironments', () => {
        it('should return environments for authorized user', async () => {
            ...
            expect(businessGetProjectEnvironmentsMock).toHaveBeenCalledWith(
                projectId,
            );
    ...
    describe('createEnvironment', () => {
        it('should create environment for authorized user', async () => {
            const projectPerms = getProjectMockInfos({
                projectPermissions: PROJECT_PERMS.MANAGE_ENVIRONMENTS,
            });
            const user = getUserMockInfos(false, undefined, projectPerms);
            authUserMock.mockResolvedValueOnce(user);

            businessCheckEnvironmentCreateMock.mockResolvedValueOnce({
                success: true,
            });
        ...
        it('should return 403 if project is locked', async () => {
            ...
            expect(response.statusCode).toEqual(403);
            expect(response.json()).toEqual({
                message: 'Le projet est verrouillé',
            });
        });

        it('should return 403 if not permited', async () => {
            const projectPerms = getProjectMockInfos({
                projectPermissions: PROJECT_PERMS.GUEST,
            });
            const user = getUserMockInfos(false, undefined, projectPerms);
            authUserMock.mockResolvedValueOnce(user);
    ...
    describe('updateEnvironment', () => {
        ...
        it('should update environment for authorized user', async () => {
            ...
            businessCheckEnvironmentUpdateMock.mockResolvedValueOnce({
                success: true,
                value: true,
            });
            businessUpdateEnvironmentMock.mockResolvedValueOnce({
                success: true,
                data: { id: environmentId, ...environmentData, ...atDates },
            });
            ...
            const response = await app
                .inject()
                .put(
                    environmentContract.updateEnvironment.path.replace(
                        ':environmentId',
                        environmentId,
                    ),
                )
    ...
    describe('deleteEnvironment', () => {
        it('should delete environment for authorized user', async () => {
            ...
            businessDeleteEnvironmentMock.mockResolvedValueOnce({
                success: true,
            });
            ...
            const response = await app
                .inject()
                .delete(
                    environmentContract.deleteEnvironment.path.replace(
                        ':environmentId',
                        environmentId,
                    ),
                )
