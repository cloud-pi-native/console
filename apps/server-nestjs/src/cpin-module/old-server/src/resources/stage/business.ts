...
export async function createStage({ clusterIds = [], name }: CreateStageBody) {
    ...
    if (isNameTaken)
        return new BadRequest400(
            "Un type d'environnement portant ce nom existe déjà",
        );
...
export async function updateStage(
    stageId: Stage['id'],
    { clusterIds, name }: UpdateStageBody,
) {
    ...
    if (dbClusters?.length) {
        const clustersToRemove = dbClusters.filter(
            (dbCluster) => !clusterIds.includes(dbCluster.id),
        );
...
export async function deleteStage(stageId: Stage['id']) {
    const attachedEnvironment = await prisma.environment.findFirst({
        where: { stageId },
        select: { id: true },
    });
    if (attachedEnvironment)
        return new BadRequest400(
            'Impossible de supprimer le stage, des environnements en activité y ont souscrit',
        );
...
export async function linkClusterToStages(
    clusterId: Cluster['id'],
    stageIds: Stage['id'][],
    linkToAll: boolean = false,
) {
    if (linkToAll === true) {
