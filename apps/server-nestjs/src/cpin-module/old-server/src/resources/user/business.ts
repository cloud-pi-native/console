    ...
    if (query.adminRoles?.length) {
        const roles = query.adminRoles
            ? await prisma.adminRole.findMany({
                  where: { name: { in: query.adminRoles } },
              })
            : [];

        const adminRoleNameNotFound = query.adminRoles?.find(
            (nameQueried) => !roles.find(({ name }) => name === nameQueried),
        );
        if (adminRoleNameNotFound) {
            return new BadRequest400(
                `Unable to find adminRole ${adminRoleNameNotFound}`,
            );
        }
        whereInputs.push({
            adminRoleIds: { hasEvery: roles.map(({ id }) => id) },
        });
    }
...
export async function patchUsers(
    users: typeof userContract.patchUsers.body._type,
) {
...
export async function logViaSession({
    ...
    const nonOidcRoleIds = matchingAdminRoles
        .filter(
            ({ oidcGroup, id }) =>
                !oidcGroup && userDb.adminRoleIds.includes(id),
        )
        .map(({ id }) => id);
    ...
    const updatedUser = await prisma.user
        .update({
            ...
            data: {
                ...user,
                adminRoleIds: nonOidcRoleIds,
                lastLogin: new Date().toISOString(),
            },
        })
        .then((user) => ({
            ...user,
            adminRoleIds: [...user.adminRoleIds, ...oidcRoleIds],
        }));
    return {
        user: updatedUser,
...
export async function logViaToken(
    ...
    return {
        ...
        adminPerms:
            token?.permissions ??
            (await getAdminRolesAndSum(token.owner.adminRoleIds)),
    };
}

function isTokenInvalid(
    token: AdminToken | PersonalAccessToken,
    ...
    if (
        ...
        currentDate.getTime() > token.expirationDate?.getTime()
    ) {
...
async function getAdminRolesAndSum(
    roles: AdminRole['id'][] | null,
): Promise<bigint> {
    if (!roles?.length) {
        return 0n;
    }
...
async function findPersonalAccessToken(
    ...
    (PersonalAccessToken & { owner: User }) | undefined | TokenInvalidReason
> {
