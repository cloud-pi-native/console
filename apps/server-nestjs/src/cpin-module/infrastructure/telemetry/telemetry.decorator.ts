...
export function StartActiveSpan(name?: string, metadata: TelemetrySpanMetadata = {}): MethodDecorator {
  return <T>(
    _target: object,
    ...
    descriptor.value = function (this: unknown, ...args: unknown[]): unknown {
      ...
      if (metadata.options) {
        return tracer.startActiveSpan(spanName, metadata.options, runInActiveSpan)
      }
      return tracer.startActiveSpan(spanName, runInActiveSpan)
    } as T
