...
import { makeCreateProjectBody } from '../src/modules/core/project/project-testing.utils'
import { ProjectService } from '../src/modules/core/project/project.service'
...
describeWithProject('ProjectService (e2e)', {}, () => {
  ...
  it('rejects ProjectService.list filter=all for non-admin', async () => {
    await expect(
      service.list({ filter: 'all' }, { userId: ownerId, adminPermissions: 0n }),
  ...
  describe('with project', () => {
    ...
    it('ProjectService.list', async () => {
      const listResult = await service.list({ filter: 'member' }, { userId: ownerId, adminPermissions: 0n })
