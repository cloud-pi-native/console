...
describeWithArgoCD('ArgoCDController (e2e)', {}, () => {
  ...
  it('should commit environment values and cleanup stale values in the zone infra repo', async () => {
    ...
    const staleAction = await gitlab.generateCreateOrUpdateAction(infraRepoId, 'main', staleFilePath, 'stale: true\n')
    await gitlab.maybeCommitActions(infraRepoId, staleAction ? [staleAction] : [], 'ci: :robot_face: Seed stale values')
  ...
  it('should update existing values and delete values of a removed environment', async () => {
    ...
    const seededActions = (await Promise.all([
      gitlab.generateCreateOrUpdateAction(infraRepoId, 'main', devFilePath, 'old: true\n'),
      gitlab.generateCreateOrUpdateAction(infraRepoId, 'main', prodFilePath, 'old: true\n'),
    ])).filter((action): action is NonNullable<typeof action> => action !== null)
    await gitlab.maybeCommitActions(infraRepoId, seededActions as CommitAction[], 'ci: :robot_face: Seed existing values')
