...
import { ArgoCDControllerService } from '@/modules/argocd/argocd-controller.service'
import { ArgoCDDatastoreService, projectSelect } from '@/modules/argocd/argocd-datastore.service'
...
describeWithArgoCD('ArgoCDController (e2e)', {}, () => {
  ...
  beforeAll(async () => {
    ...
    gitlab = moduleRef.get<GitlabService>(GitlabService)
    gitlabClient = moduleRef.get<GitlabClientService>(GitlabClientService)
    vault = moduleRef.get<VaultService>(VaultService)
  ...
  afterAll(async () => {
    if (vaultProjectValuesPath) {
      await vault.destroy(vaultProjectValuesPath).catch(() => {})
    }
  ...
  it('should commit environment values and cleanup stale values in the zone infra repo', async () => {
    ...
    await gitlab.maybeCommitUpdate(
      infraRepoId,
      [{ content: 'stale: true\n', filePath: staleFilePath }],
      'ci: :robot_face: Seed stale values',
    )
    ...
    const shouldBeDeleted = await gitlab.getFile(infraRepoId, staleFilePath, 'main')
  ...
  it('should update existing values and delete values of a removed environment', async () => {
    ...
    await gitlab.maybeCommitUpdate(
      infraRepoId,
      [
        { filePath: devFilePath, content: 'old: true\n' },
        { filePath: prodFilePath, content: 'old: true\n' },
      ],
      'ci: :robot_face: Seed existing values',
    )
    ...
    const prodFile = await gitlab.getFile(infraRepoId, prodFilePath, 'main')
