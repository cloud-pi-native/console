...
describeWithProjectServices('ProjectServicesService (e2e)', {}, () => {
  ...
  it('getProjectServices returns the registered service for a project member', async () => {
    ...
    const services = await service.getProjectServices(projectId, 'user')
  ...
  it('rejects getProjectServices when project does not exist', async () => {
    await expect(service.getProjectServices(faker.string.uuid(), 'user')).rejects.toThrow(NotFoundException)
  })

  it('updateProjectServices stores project configuration', async () => {
    await service.updateProjectServices(projectId, {
      [TEST_PLUGIN_NAME]: {
        enabled: 'enabled',
      },
    }, ['user'])
