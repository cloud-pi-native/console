...
import { RegistryHttpClientService } from '../src/modules/registry/registry-http-client.service'
import { getHostFromUrl, getProjectVaultPath } from '../src/modules/registry/registry.utils'
import { VaultClientService } from '../src/modules/vault/vault-client.service'
import { VaultHttpClientService } from '../src/modules/vault/vault-http-client.service'
...
describeWithRegistry('RegistryClientService (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    projectSlug = faker.helpers.slugify(`e2e-registry-${faker.string.alphanumeric({ length: 10 }).toLowerCase()}`).slice(0, 50)
  })

  afterAll(async () => {
    if (vault && config && projectSlug) {
      const paths = [roRobotName, rwRobotName, projectRobotName].map(name => getProjectVaultPath(config.projectRootDir, projectSlug, `REGISTRY/${name}`))
      await Promise.all(paths.map(path => vault.delete(path).catch(() => {})))
    }

    if (client && projectSlug) {
      await client.deleteProject(projectSlug).catch(() => {})
    }
  ...
  it('should provision project in Harbor and write robot secrets to Vault', async () => {
    ...
    const result = await client.provisionProject(projectSlug, { publishProjectRobot: true })
    ...
    const robotNames = Array.isArray(robots.data) ? robots.data.map((r: any) => r?.name).filter(Boolean) : []
    ...
    const vaultPaths = [roRobotName, rwRobotName, projectRobotName].map(name => getProjectVaultPath(config.projectRootDir, projectSlug, `REGISTRY/${name}`))
