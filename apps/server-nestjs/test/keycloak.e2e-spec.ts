...
describeWithKeycloak('KeycloakController (e2e)', () => {
  ...
  beforeAll(async () => {
    moduleRef = await Test.createTestingModule({
      imports: [KeycloakModule, ConfigModule.forRoot({ envFilePath: [...(process.env.DOCKER !== 'true' ? ['.env'] : []), ...(process.env.INTEGRATION === 'true' ? ['.env.integ'] : [])], isGlobal: true, load: [baseConfig] }), AuthModule, DatabaseModule, EventsModule, LoggerModule, PermissionModule],
    }).compile()
