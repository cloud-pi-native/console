...
import { KeycloakControllerService } from '../src/modules/keycloak/keycloak-controller.service'
...
describe('KeycloakController (e2e)', () => {
  ...
  beforeAll(async () => {
    // Ensure environment variables are set for Keycloak connection
    vi.stubEnv('KEYCLOAK_DOMAIN', process.env.KEYCLOAK_DOMAIN || 'localhost:8090')
    vi.stubEnv('KEYCLOAK_REALM', process.env.KEYCLOAK_REALM || 'cloud-pi-native')
    vi.stubEnv('KEYCLOAK_PROTOCOL', process.env.KEYCLOAK_PROTOCOL || 'http')
    vi.stubEnv('KEYCLOAK_ADMIN', process.env.KEYCLOAK_ADMIN || 'admin')
    vi.stubEnv('KEYCLOAK_ADMIN_PASSWORD', process.env.KEYCLOAK_ADMIN_PASSWORD || 'admin')
  ...
  it('should reconcile and create groups in Keycloak', async () => {
    ...
    const projectGroup = await keycloakService.getGroupByPath(`/${testProjectSlug}`)
    expect(projectGroup).toBeDefined()
    expect(projectGroup?.name).toBe(testProjectSlug)

    // Check role group
    const roleGroup = await keycloakService.getGroupByPath(`/${testProjectSlug}/${testRoleName}`)
  ...
  it('should recreate project group if deleted in Keycloak', async () => {
    ...
    let projectGroup = await keycloakService.getGroupByPath(`/${testProjectSlug}`)
    expect(projectGroup).toBeDefined()
    ...
    // Verify deleted
    projectGroup = await keycloakService.getGroupByPath(`/${testProjectSlug}`)
  ...
  it('should recreate role group if deleted in Keycloak', async () => {
    ...
    let roleGroup = await keycloakService.getGroupByPath(`/${testProjectSlug}/${testRoleName}`)
    expect(roleGroup).toBeDefined()
    ...
    // Verify deleted
    roleGroup = await keycloakService.getGroupByPath(`/${testProjectSlug}/${testRoleName}`)
    expect(roleGroup).toBeUndefined()
