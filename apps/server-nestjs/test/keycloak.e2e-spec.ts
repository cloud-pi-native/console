...
describeWithKeycloak('KeycloakController (e2e)', () => {
  ...
  it('should reconcile and create groups in Keycloak', async () => {
    ...
    const consoleGroup = z.object({
      id: z.string(),
      name: z.string(),
    }).parse(await keycloakService.getGroupByPath(`/${testProjectSlug}/console`))
    expect(consoleGroup.name).toBe('console')

    // Check role group
    const roleGroup = z.object({
      name: z.string(),
    }).parse(await keycloakService.getGroupByPath(`/${testProjectSlug}/console/${testRoleName}`))
  ...
  it('should recreate role group if deleted in Keycloak', async () => {
    ...
    // Verify deleted
    const deletedRoleGroup = await keycloakService.getGroupByPath(`/${testProjectSlug}/console/${testRoleName}`)
