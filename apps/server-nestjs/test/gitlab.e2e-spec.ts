...
import { GitlabControllerService } from '../src/modules/gitlab/gitlab-controller.service'
...
describeWithGitLab('GitlabController (e2e)', {}, () => {
  ...
  beforeAll(async () => {
    ...
    await vaultService.write({ ok: true }, vaultProbePath)
    await vaultService.read(vaultProbePath)
  ...
  afterAll(async () => {
    if (vaultProbePath) {
      await vaultService.delete(vaultProbePath).catch(() => {})
    }
