...
export const getRepositoryByIdController = async (req, res) => {
  ...
  try {
    ...
    if (!hasRoleInProject(userId, { roles: project.roles })) throw new Error('Vous n\'êtes pas membre du projet')
...
export const getProjectRepositoriesController = async (req, res) => {
  ...
  try {
    ...
    if (!hasRoleInProject(userId, { roles })) throw new Error('Vous n\'êtes pas membre du projet')
...
export const createRepositoryController = async (req: EnhancedFastifyRequest<CreateRepositoryDto>, res) => {
  ...
  try {
    ...
    if (!hasRoleInProject(user.id, { roles: project.roles, minRole: 'owner' })) return sendForbidden(res, 'Vous n\'êtes pas membre du projet')
...
export const deleteRepositoryController = async (req: EnhancedFastifyRequest<DeleteRepositoryDto>, res) => {
  ...
  try {
    project = await getProjectInfos(projectId)
    if (!hasRoleInProject(userId, { roles: project.roles, minRole: 'owner' })) return sendForbidden(res, 'Vous n\'êtes pas souscripteur du projet')
    repo = await getRepositoryById(repositoryId)
