...
if (process.env.DOCKER !== 'true' && fs.existsSync('.env')) {
  Object.assign(process.env, Object.fromEntries(
    Object.entries(parseEnv(fs.readFileSync(path.resolve('.env'), 'utf-8'))).filter(Boolean),
  ))
}

if (process.env.INTEGRATION === 'true' && fs.existsSync('.env.integ')) {
  Object.assign(process.env, Object.fromEntries(
    Object.entries(parseEnv(fs.readFileSync(path.resolve('.env.integ'), 'utf-8'))).filter(Boolean),
  ))
}
