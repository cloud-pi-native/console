...
describe('authUser effective admin permissions via refined permissions', () => {
  beforeEach(() => {
    vi.resetAllMocks()
  })

  it('returns legacy permissions when refined-permissions is off', async () => {
  ...
  it('does not add ManageProjects when refined-permissions is on and MANAGE not set', async () => {
    ...
    const body = response.json() as { adminPermissions: string }
    expect(body.adminPermissions).toEqual(rawPerms.toString())
