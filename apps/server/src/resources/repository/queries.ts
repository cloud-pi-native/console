...
> &
  Partial<
    Pick<
      Repository,
      'externalUserName' | 'externalRepoUrl' | 'deployRevision' | 'deployPath' | 'helmValuesFiles'
    >
  >;
...
export async function deleteRepository(id: Repository['id']) {
  ...
  if (!doesRepoExist)
    throw new Error("Le dépôt interne demandé n'existe pas en base pour ce projet");
