...
import { BadRequest400, ErrorResType, Unprocessable422 } from '../../utils/errors.js';
...
describe('test project business logic', () => {
  ...
  describe('listProjects', () => {
    it('should return stringified perms', async () => {
      prisma.project.findMany.mockResolvedValue([
        { everyonePerms: 5n, clusters: [], roles: [{ permissions: 28n }] },
      ]);
  ...
  describe('updateProject', () => {
    ...
    const members: ProjectMembers[] = [
      { userId: faker.string.uuid(), projectId: project.id, roleIds: [], user: { type: 'human' } },
    ];
    it('should update project', async () => {
      ...
      await updateProject(
        { ...updatedProjet, ownerId: members[0].userId },
        project.id,
        user,
        reqId,
      );
  ...
  describe('archiveProject', () => {
    it('should archive project', async () => {
      ...
      hook.project.delete.mockResolvedValue({
        results: { failed: false },
        project: Promise.resolve({ status: 'archived' }),
      });
...
describe('chunk function', () => {
  ...
  it('should return 3,2 elements', () => {
    const letters = ['A', 'B', 'C', 'D', 'E'];
    expect(chunk(letters, 3)).toEqual([
      ['A', 'B', 'C'],
      ['D', 'E'],
    ]);
  });
