  ...
  return listProjectsQuery({
    ...
    projects.map(({ clusters, ...project }) => ({
      ...
      roles: project.roles.map((role) => ({
        ...
        oidcGroup: project.slug ? role.oidcGroup?.replace(`/${project.slug}`, '') : role.oidcGroup,
      })),
...
export async function createProject(
  ...
  if (requestor.type !== 'human')
    return new BadRequest400('Seuls les comptes humains peuvent créer des projets');
  ...
  logger.info(
    { requestId, userId: requestor.id, projectId: project.id, slug },
    'Project initialized',
  );
  ...
  if (results.failed) {
    logger.error(
      { requestId, userId: requestor.id, projectId: project.id, slug },
      'Create project failed during upsert hooks',
    );
  } else {
    logger.info(
      { requestId, userId: requestor.id, projectId: project.id, slug },
      'Create project upsert hooks completed',
    );
  }
  if (results.failed) {
  ...
  return {
    ...
    roles: projectInfos.roles.map((role) => ({
      ...role,
      permissions: role.permissions.toString(),
      oidcGroup: projectInfos.slug
        ? role.oidcGroup?.replace(`/${project.slug}`, '')
        : role.oidcGroup,
    })),
  };
}
...
export async function updateProject(
  ...
  const changedFields = Object.entries({
    ...
    .map(([key]) => key);
  logger.info(
    { requestId, userId: requestor.id, projectId, changedFields },
  ...
  if (ownerIdCandidate && ownerIdCandidate !== projectDb.ownerId) {
    ...
    if (memberCandidate.user.type !== 'human')
      return new BadRequest400('Seuls les comptes humains peuvent être propriétaire de projets');
  ...
  if (results.failed) {
    logger.error(
      { requestId, userId: requestor.id, projectId },
      'Update project failed during upsert hooks',
    );
  } else {
    logger.info(
      { requestId, userId: requestor.id, projectId },
      'Update project upsert hooks completed',
    );
  }
  if (results.failed) {
  ...
  return {
    ...
    roles: projectInfos.roles.map((role) => ({
      ...
      permissions: role.permissions.toString(),
      oidcGroup: projectInfos.slug
        ? role.oidcGroup?.replace(`/${projectInfos.slug}`, '')
        : role.oidcGroup,
    })),
  };
}
...
export async function archiveProject(
  ...
  if (results.failed) {
    logger.error(
      { requestId, userId: requestor.id, projectId },
      'Archive project failed during delete hooks',
    );
  } else {
    logger.info(
      { requestId, userId: requestor.id, projectId, status: project.status },
      'Archive project delete hooks completed',
    );
  }
