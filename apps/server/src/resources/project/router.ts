  ...
  return serverInstance.router(projectContract, {
    ...
    createProject: async ({ request: req, body: data }) => {
      ...
      if (perms.user?.type !== 'human')
        return new Unauthorized401('Cannot find requestor in database');
    ...
    getProject: async ({ request: req, params }) => {
      ...
      if (!perms.projectPermissions && !AdminAuthorized.Manage(perms.adminPermissions))
        return new Forbidden403();
    ...
    updateProject: async ({ request: req, params, body: data }) => {
      ...
      if (perms.projectLocked) {
        ...
        if (data.locked !== false)
          return new Forbidden403('Veuillez déverrouiler le projet pour le mettre à jour');
      }
