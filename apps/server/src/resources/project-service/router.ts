...
export function projectServiceRouter() {
  return serverInstance.router(projectServiceContract, {
    ...
    getServices: async ({ request: req, params: { projectId }, query }) => {
      ...
      if (query.permissionTarget === 'admin' && !AdminAuthorized.Manage(perms.adminPermissions))
        return new Forbidden403('Vous ne pouvez pas demander les paramètres admin');
    ...
    updateProjectServices: async ({ request: req, params: { projectId }, body }) => {
      ...
      const allowedRoles: Array<'user' | 'admin'> = AdminAuthorized.Manage(perms.adminPermissions)
        ? ['user', 'admin']
        : ['user'];
