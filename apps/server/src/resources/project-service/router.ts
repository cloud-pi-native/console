...
export function projectServiceRouter() {
  ...
  // Récupérer les services d'un projet
    getServices: async ({ request: req, params: { projectId }, query }) => {
      ...
      if (query.permissionTarget === 'admin' && !AdminAuthorized.ManageProjects(perms.adminPermissions)) return new Forbidden403('Vous ne pouvez pas demander les paramètres admin')

      const target = query.permissionTarget ?? 'user'
      const body = await getProjectServices(projectId, target)
