...
import type { PermissionTarget, PluginsUpdateBody, ServiceUrl } from '@cpn-console/shared';
...
export function objToDb(obj: PluginsUpdateBody): ConfigRecords {
  return Object.entries(obj)
    .map(([pluginName, values]) =>
      Object.entries(values).map(([key, value]) => ({ pluginName, key, value })),
    )
    .flat();
}
...
export async function updateProjectServices(
  projectId: Project['id'],
  data: PluginsUpdateBody,
  stripperRoles: Array<'user' | 'admin'>,
) {
  const pluginKeysCount = Object.fromEntries(
    Object.entries(data).map(([pluginName, values]) => [pluginName, Object.keys(values).length]),
  );
  logger.info(
    { projectId, pluginCount: Object.keys(data).length, pluginKeysCount, stripperRoles },
    'Update project services started',
  );
