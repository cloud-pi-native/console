...
export async function createToken(data: typeof adminTokenContract.createAdminToken.body._type) {
  ...
  logger.info(
    { tokenName: data.name, expirationDate: data.expirationDate ?? null },
    'Create admin token started',
  );
  ...
  logger.info(
    {
      ...
      botUserId,
      expirationDate: token.expirationDate ?? null,
      status: token.status,
    },
    'Create admin token completed',
  );
  return {
    ...token,
    password,
