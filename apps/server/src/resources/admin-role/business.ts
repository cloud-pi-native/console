...
export async function listRoles() {
  return listAdminRoles()
    .then(roles => roles.map(role => ({ ...role, permissions: role.permissions.toString(), type: (role as any).type ?? 'custom' })))
}
...
export async function deleteRole(roleId: Project['id']) {
  ...
  if (role) {
    if ((role as any).type === 'system') return new Forbidden403('Impossible de supprimer un rôle système')
