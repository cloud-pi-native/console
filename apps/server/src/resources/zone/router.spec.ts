...
describe('test zoneContract', () => {
  ...
  describe('listZones', () => {
    it('should return list of zones', async () => {
      ...
      const response = await app.inject().get(zoneContract.listZones.path).end();
  ...
  describe('createZone', () => {
    const zone = {
      ...
      label: faker.string.alpha({ length: 5 }),
    ...
    it('should create and return zone for admin', async () => {
      ...
      const response = await app.inject().post(zoneContract.createZone.path).body(zone).end();
