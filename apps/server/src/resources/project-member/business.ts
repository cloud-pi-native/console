...
export async function addMember(
  ...
  logger.info(
    { requestId, requestorId, projectId, identifierType: user.userId ? 'userId' : 'email' },
    'Add project member started',
  );
  if (user.userId) {
  ...
  if (userInDb) {
    if (userInDb.id === projectOwnerId)
      return new BadRequest400('Le owner ne peut pas être ajouté à cette liste');
  } else if (user.email) {
    ...
    if (hookReply.failed) {
      logger.error(
        { requestId, requestorId, projectId },
        'Add project member failed during user lookup hooks',
      );
  ...
  logger.info(
    { requestId, requestorId, projectId, userId: userInDb.id },
    'Add project member completed',
  );
