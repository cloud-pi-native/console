...
async function addEnvironment(environment: Omit<Environment, 'id' | 'projectId'>) {
  ...
  if (projectStore.selectedProject && !projectStore.selectedProject.locked) {
    const envs = await projectEnvironmentStore.addEnvironmentToProject({ ...environment, projectId: projectStore.selectedProject.id })
    projectStore.myProjectsById[projectStore.selectedProject.id].environments = envs
  }
  cancel()
...
async function putEnvironment(environment: Pick<Environment, 'quotaId' | 'id'>) {
  snackbarStore.isWaitingForResponse = true
  if (projectStore.selectedProject && !projectStore.selectedProject.locked) {
    const envs = await projectEnvironmentStore.updateEnvironment(environment.id, environment)
    projectStore.myProjectsById[projectStore.selectedProject.id].environments = envs
  }
  cancel()
  snackbarStore.isWaitingForResponse = false
...
async function deleteEnvironment(environmentId: Environment['id']) {
  snackbarStore.isWaitingForResponse = true
  const envs = await projectEnvironmentStore.deleteEnvironment(environmentId)
  projectStore.myProjectsById[projectStore.selectedProject.id].environments = envs
...
<template>
  <template
    v-if="ProjectAuthorized.ListRepositories({ projectPermissions: projectStore.selectedProject.myPerms })"
  >
    <template
