...
const projectList = computed(() =>
  projectStore.myProjects
    .map((project) => ({
      ...
      details:
        userStore.userProfile?.id !== project.owner.id
...
<template>
  ...
  <div class="gap-8 flex flex-wrap">
    <div v-for="project in projectList" id="project-list" :key="project.id" class="min-w-75 h-45">
      <DsfrTile
        class="h-full"
        v-bind="project"
        :title="project.name"
    ...
    <template v-if="!projectList.length">
      <div>Vous ne faites parti d'aucun projet pour l'instant</div>
    </template>
  </div>
</template>
