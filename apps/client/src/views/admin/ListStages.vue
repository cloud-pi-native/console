...
function setStageTiles(stages: Stage[]) {
  stageList.value = sortArrByObjKeyAsc(stages, 'name')?.map((stage) => ({
    id: stage.id,
    title: stage.name,
    data: stage,
  }));
...
<template>
  ...
  <div v-if="isNewStageForm" class="my-5 pb-10 border-grey-900 border-y-1">
    <StageForm
  ...
  >
    <div v-for="stage in stageList" :key="stage.id" class="flex-basis-60 flex-stretch max-w-90">
      <DsfrTile
        :title="stage.title"
