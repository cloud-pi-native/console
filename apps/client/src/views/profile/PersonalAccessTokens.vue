...
<template>
  ...
  <div>
    <DsfrTable data-testid="tokenTable" title="" no-caption :headers="headers">
      <tr v-if="!tokens.length">
        <td :colspan="headers.length">Aucune clé d'api existante</td>
      </tr>
      <tr v-for="token in tokens" v-else :key="token.id" :data-testid="`token-${token.id}`">
        <td>{{ token.name }}</td>
