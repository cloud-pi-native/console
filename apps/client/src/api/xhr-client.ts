...
export const apiClient = await getApiClient(
  ...
  async (
    ...
    status: number;
    body: { status: number; error: string } | unknown;
    headers: Headers;
  }> => {
...
export function extractData<
  ...
  S extends T['status'],
>(response: T, expectedStatus: S): Extract<T, { status: S }>['body'] {
  ...
  try {
    throw new Error(
      `Erreur lors de la requete, reçu ${response.status}, attendu ${expectedStatus}`,
    );
  } catch (error: unknown) {
