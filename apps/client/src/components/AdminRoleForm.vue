...
const usersInRole = computed(() =>
  users.value.filter((user) => user.adminRoleIds.includes(role.value.id)),
);
...
const isUserAlreadyInTeam = computed(() => {
  return !!(
    newUserInput.value && usersInRole.value.find((member) => member.id === newUser.value?.id)
  );
});
...
async function switchUserMembership(checked: boolean, user: User, fromSuggestion = false) {
  ...
  const response = await usersStore.patchUsers([
    { id: user.id, adminRoleIds: newUserAdminRoleIds },
  ]);
  ...
  >
    <DsfrTabContent panel-id="general" tab-id="general">
      <DsfrInput
        ...
        :disabled="isSystem"
      />
      <p class="fr-h6">Permissions</p>
      <div v-for="scope in adminPermsDetails" :key="scope.name">
