...
import type { LettersQuery, User } from '@cpn-console/shared';
...
const headers = ['Identifiant', 'E-mail', 'Rôles', 'Retirer du projet'];
...
const isUserAlreadyInTeam = computed(() => {
  return !!(
    newUserEmail.value &&
    (props.project.owner.email === newUserEmail.value ||
      props.project.members.find((member) => member.email === newUserEmail.value))
  );
});

const usersToSuggest = computed(() =>
  usersToAdd.value.map((userToAdd) => ({
    value: userToAdd.email,
    furtherInfo: `${userToAdd.firstName} ${userToAdd.lastName}`,
  })),
);

const getRolesNames = (ids?: string[]) =>
  ids
    ? props.project.roles
...
const retrieveUsersToAdd = pDebounce(async (letters: LettersQuery['letters']) => {
  ...
  if (
    lettersNotMatching.value &&
    letters.includes(lettersNotMatching.value) &&
    !usersToAdd.value?.length
  )
    return;
  usersToAdd.value =
    await projectStore.projectsBySlug[props.project.slug].Members.getCandidateUsers(letters);
...
async function addUserToProject() {
  ...
  await props.project.Members.create(newUserEmail.value).finally(() => emit('refresh'));
...
async function removeUserFromProject(userId: string) {
  await props.project.Members.delete(userId).finally(() => {
    emit('refresh');
...
async function transferOwnerShip() {
  if (
    nextOwnerId.value &&
    props.project.members.some((member) => member.userId === nextOwnerId.value)
  ) {
...
<template>
  <div class="flex flex-col w-max">
    ...
    >
      <tr
        v-for="member in [
          { ...props.project.owner, roleIds: [], userId: props.project.owner.id },
          ...project.members,
        ]"
        :key="member.userId"
      >
        ...
        <td>{{ member.email }}</td>
        <td>
          {{ project.ownerId === member.userId ? 'Propriétaire' : getRolesNames(member.roleIds) }}
        </td>
        <td>
          <div v-if="props.project.ownerId === member.userId">-</div>
          <div
    ...
    />
    <div v-if="canTransfer" data-testid="transferProjectZone" class="danger-zone">
      <DsfrButton
      ...
      <div v-if="isTransferingProject">
        ...
        <div v-else>
          ...
          <div class="flex justify-between">
            ...
            />
            <DsfrButton label="Annuler" primary @click="isTransferingProject = false" />
          </div>
        </div>
      </div>
    </div>
