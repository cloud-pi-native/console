...
import {
  ...
  keycloakLogin,
  keycloakLogout,
} from '@/utils/keycloak/keycloak.js';
...
export const useUserStore = defineStore('user', () => {
  ...
  const myAdminRoles = computed<AdminRole[]>(
    () =>
      adminRoleStore.roles?.filter((adminRole) =>
        apiAuthInfos.value?.adminRoleIds.includes(adminRole.id),
      ) ?? [],
  );
  ...
  const setUserProfile = async () => {
    ...
    await apiClient.Users.auth().then((res: any) => (apiAuthInfos.value = extractData(res, 200)));
  };
