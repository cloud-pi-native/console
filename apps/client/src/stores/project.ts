...
export const useProjectStore = defineStore('project', () => {
  ...
  const amiIPartOf = (project: ProjectV2) => project.status !== 'archived'
  ...
  // TODO restore alphebatical sort
  const projects = computed(() => Object.values(projectsById.value),
    // .sort((p1, p2) => p1.name.localeCompare(p2.name))
  )

  const myProjects = computed(() => projects.value.filter(project => project.status !== 'archived' && amiIPartOf(project)))
