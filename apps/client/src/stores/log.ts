...
export const useLogStore = defineStore('log', () => {
  ...
  const getAllLogs = async ({ offset, limit }: GetLogsQuery = { offset: 0, limit: 100 }) => {
    const res = await apiClient.Logs.getLogs({ query: { offset, limit, clean: false } }).then(
  ...
  const listLogs = async (
    { offset, limit, clean, projectId }: GetLogsQuery = { offset: 0, limit: 10 },
  ) => {
    return apiClient.Logs.getLogs({ query: { offset, limit, clean, projectId } }).then(
      (response: any) => extractData(response, 200),
    );
  };
