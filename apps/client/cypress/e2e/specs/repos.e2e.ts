...
describe('Add repos into project', () => {
  ...
  it('Should handle repository schema validation', () => {
    ...
    cy.getByDataTestid('addRepoBtn').should('be.enabled')
    cy.getByDataTestid('internalRepoNameInput').clear().type('$%_>').get('.fr-error-text').should('have.length', 1).and('contain', invalidInternalRepoName)
    cy.getByDataTestid('addRepoBtn').should('be.disabled')
    cy.getByDataTestid('internalRepoNameInput').clear().type(repo.internalRepoName).get('.fr-error-text').should('not.exist')
    cy.getByDataTestid('addRepoBtn').should('be.enabled')
    cy.getByDataTestid('externalRepoUrlInput').clear().type(repo.externalRepoUrl.slice(0, -4)).get('.fr-error-text').should('have.length', 1).and('contain', invalidGitUrl)
    cy.getByDataTestid('addRepoBtn').should('be.disabled')
    cy.getByDataTestid('externalRepoUrlInput').clear().type(repo.externalRepoUrl.slice(8)).get('.fr-error-text').should('have.length', 2).and('contain', invalidGitUrl)
    cy.getByDataTestid('addRepoBtn').should('be.disabled')
    cy.getByDataTestid('externalRepoUrlInput').clear().type(repo.externalRepoUrl.replace(/s/i, '')).get('.fr-error-text').should('have.length', 1).and('contain', invalidGitUrl)
    cy.getByDataTestid('addRepoBtn').should('be.disabled')
    cy.getByDataTestid('externalRepoUrlInput').clear().type(repo.externalRepoUrl).get('.fr-error-text').should('not.exist')
    cy.getByDataTestid('addRepoBtn').should('be.enabled')
  ...
  it('Should add a standalone public repo', () => {
    ...
    cy.getByDataTestid('addRepoLink').click().get('h2').should('contain', 'Ajouter un dépôt au projet').getByDataTestid('internalRepoNameInput').type(repo.internalRepoName)
    cy.getByDataTestid('standaloneRepoSwitch')
      .find('input')
      .check({ force: true })
  ...
  it('Should update a repo', () => {
    ...
    cy.getByDataTestid(`repoTile-repo03`).click().get('h2').should('contain', 'Modifier le dépôt').getByDataTestid('internalRepoNameInput').should('be.disabled').getByDataTestid('externalRepoUrlInput').clear().type('https://github.com/externalUser04/new-repo.git')

    cy.getByDataTestid('input-checkbox-privateRepoCbx').check({ force: true }).getByDataTestid('externalUserNameInput').type('newUser').getByDataTestid('externalTokenInput').clear().type('newToken')
  ...
  it('Should not be able to delete a repository if not owner', () => {
    ...
    cy.goToProjects()
    cy.get('[data-testid^="projectTile-"]:first').click().getByDataTestid('menuRepos').click()

    cy.get('[data-testid^="repoTile-"]:first').click().getByDataTestid('repo-form').should('exist').getByDataTestid('deleteRepoZone').should('not.exist')
  })

  it('Should not be able to delete a repository if project locked', () => {
    cy.goToProjects()
      .getByDataTestid(`projectTile-${projectFailed.name}`)
      .click()
      .getByDataTestid('menuRepos')
      .click()

    cy.get('[data-testid^="repoTile-"]:first').click().getByDataTestid('repo-form').should('exist')
