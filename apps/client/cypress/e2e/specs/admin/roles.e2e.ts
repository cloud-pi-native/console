...
describe('Administration roles', () => {
  ...
  it('Should add a new oidc role', () => {
    cy.get('[data-testid$="-tab"]').contains(newOidcRole.name).should('not.exist')
    ...
    cy.wait('@patchRole')
      .its('response.statusCode')
      .should('match', /^20\d$/)
    cy.get('[data-testid$="-tab"]').contains(newOidcRole.name).should('be.visible')
  ...
  it('Should add a new non-oidc role', () => {
    cy.get('[data-testid$="-tab"]').contains(newRole.name).should('not.exist')
    cy.getByDataTestid('addRoleBtn')
      .should('be.enabled')
      .click()
    cy.wait('@createRole')
    ...
    cy.wait('@patchRole')
      .its('response.statusCode')
      .should('match', /^20\d$/)
    cy.get('[data-testid$="-tab"]').contains(newRole.name).should('be.visible')
  })

  it('Should add a user to a role', () => {
    cy.get('[data-testid$="-tab"]').contains(newRole.name).click()
    cy.getByDataTestid('test-members')
      .click()
    ...
    cy.getByDataTestid('tableAdministrationUsers').contains(newRole.users[0].email).parent().should('contain', newRole.name)
  })
})
...
describe('Administration roles, vérification en tant que nouvel admin', () => {
  ...
  it('Should remove oidc role', () => {
    cy.get('[data-testid$="-tab"]').contains(newOidcRole.name).should('be.visible').click()
...
describe('Administration roles', () => {
  ...
  it('Should remove non oidc role', () => {
    cy.get('[data-testid$="-tab"]').contains(newRole.name).should('be.visible').click()
    cy.getByDataTestid('deleteBtn')
      .should('be.enabled')
      .click()
    cy.wait('@deleteRole')
    cy.goToAdminListUsers()
    cy.getByDataTestid('tableAdministrationUsers').contains(newRole.users[0].email).parent().should('not.contain', newRole.name)
  })
})
