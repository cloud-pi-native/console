...
describe('Create Project', () => {
  ...
  it('Should create a project with minimal form informations', () => {
    ...
    cy.goToProjects()
      ...
      .should('not.exist')
    cy.getByDataTestid('ownerInfo').should('contain', owner.email).get('select#organizationId-select').select(project.orgId).getByDataTestid('nameInput').type(`${project.name} ErrorSpace`).getByDataTestid('nameInput').should('have.class', 'fr-input--error').getByDataTestid('createProjectBtn').should('be.disabled').getByDataTestid('nameInput').clear().type(project.name).getByDataTestid('nameInput').should('not.have.class', 'fr-input--error').getByDataTestid('createProjectBtn').should('be.enabled').getByDataTestid('descriptionInput').clear().type(project.description)
  ...
  it('Should not create a project if name is already taken', () => {
    ...
    cy.goToProjects()
      ...
      .select(project.orgId)
      .getByDataTestid('nameInput')
      .type(`${project.name}`)
    cy.getByDataTestid('createProjectBtn').should('be.enabled').click()
