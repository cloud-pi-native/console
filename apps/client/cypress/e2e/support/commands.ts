...
Cypress.Commands.add('kcLogin', (name, password = 'test') => {
  cy.session(name, () => {
    cy.visit('/')
      ...
      .click()
      .url()
      .should('contain', `${Cypress.env('clientHost')}`)
  }, {
    validate() {
      cy.visit('/')
        .get('a.fr-btn')
        .should('contain', 'Se déconnecter')
    },
  })
})
...
Cypress.Commands.add('createProject', (project, ownerEmail = defaultOwner.email) => {
  ...
  cy.goToProjects()
    ...
    .click()
    .get('h1')
    .should('contain', 'Commander un espace projet')
    .get('[data-testid^="repoFieldset-"]')
    .should('not.exist')
  cy.getByDataTestid('ownerInfo').should('contain', ownerEmail).get('select#organizationId-select').select(orgMi.id).getByDataTestid('nameInput').clear().type(newProject.name).getByDataTestid('nameInput').should('not.have.class', 'fr-input--error')
...
Cypress.Commands.add('archiveProject', (project) => {
  ...
  cy.wait('@getAllStages')
    .getByDataTestid('archiveProjectInput')
    .should('not.exist')
    .getByDataTestid('archiveProjectZone')
    .should('be.visible')
...
Cypress.Commands.add('addRepos', (project, repos) => {
  ...
  cy.goToProjects()
    .getByDataTestid(`projectTile-${project.name}`)
    .click()
    .getByDataTestid('menuRepos')
    .click()
    .url()
    .should('contain', '/repositories')

  newRepos.forEach((repo) => {
    cy.getByDataTestid('addRepoLink').click().get('h2').should('contain', 'Ajouter un dépôt au projet').getByDataTestid('internalRepoNameInput').type(repo.internalRepoName).getByDataTestid('externalRepoUrlInput').clear().type(repo.externalRepoUrl)
...
Cypress.Commands.add('deleteRepo', (project, repo) => {
  ...
  cy.getByDataTestid(`repoTile-${repo.internalRepoName}`).click().getByDataTestid('repo-form').should('exist').getByDataTestid('deleteRepoInput').should('not.exist').getByDataTestid('deleteRepoZone').should('be.visible').getByDataTestid('showDeleteRepoBtn').click().getByDataTestid('deleteRepoBtn').should('be.disabled').getByDataTestid('deleteRepoInput').should('be.visible').type(repo.internalRepoName).getByDataTestid('deleteRepoBtn').should('be.enabled').click().getByDataTestid('repo-form').should('not.exist').reload().getByDataTestid(`repoTile-${repo.internalRepoName}`).should('not.exist')
})
...
Cypress.Commands.add('deleteEnvironment', (project, environmentName) => {
  ...
  cy.getByDataTestid('deleteEnvironmentInput').clear().type(environmentName)
  cy.getByDataTestid('deleteEnvironmentBtn').should('be.enabled').click()
...
Cypress.Commands.add('assertPermission', (project, environmentName, permissions) => {
  ...
  cy.goToProjects()
    .getByDataTestid(`projectTile-${project.name}`)
    .click()
    .getByDataTestid('menuEnvironments')
    .click()
  cy.wait('@listEnvironments')
  cy.wait('@getClusters')
  cy.getByDataTestid(`environmentTile-${environmentName}`)
...
Cypress.Commands.add('goToAdminListUsers', () => {
  cy.visit('/')
    .getByDataTestid('menuAdministrationBtn')
    .click()
    .getByDataTestid('menuAdministrationUsers')
    .click()
    .url()
